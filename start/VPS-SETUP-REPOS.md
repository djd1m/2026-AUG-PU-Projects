...
⚠️ **Это не альтернатива стеку курса.** Проекты курса идут по Architecture Constraints
`p-replicator`: распределённый монолит, Docker Compose, Postgres в контейнере. Репозиторий
ценен другим — обоснованием выбора **российского** провайдера и разбором доступности из РФ,
и это применимо независимо от способа деплоя. Сам паттерн rsync+PM2 в проектах курса не используем.
