...
2. If calendar_locked_at nonnull, reject different timezone, including paused program. Expected latest version must equal persisted max (0 when absent). Explicit future effective_at must be strictly greater than server now and all existing effective_at; effective_mode=now uses transaction statement time and must follow latest version time. This prevents retroactive insertion and ambiguous ordering; cancel/edit of scheduled versions is out of scope.
