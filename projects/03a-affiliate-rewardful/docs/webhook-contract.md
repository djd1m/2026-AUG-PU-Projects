...
| Business refund | Refund unique(connection_id,provider_refund_id), linked parent; RefundAllocationRevision unique(payment,canonical refund_set_hash) plus unique(revision,basis_month) append-only difference |
| Registration | Attribution unique(connection_id,customer_id,project_id), immutable registration_payload_hash and registration-time EligibilityFact IDs; delayed delivery uses historical status, not current revocation |
...
Payment occurred_at comes from canonical `payment.captured_at`; refund occurred_at uses canonical `refund.created_at`, explicitly meaning creation time of the subsequently verified succeeded refund. `event_time_basis` names this distinction; missing/invalid/future timestamp is an exception, not replaced by arrival. N1 delivery sequence/cursor orders recovery, not monetary truth. Cancel/nonterminal/older events never roll back a succeeded payment or paid_until. Refund without parent waits/reconciles; partial refunds lock parent and recompute cumulative marginal rounding over ALL known refunds sorted by occurred_at/provider_refund_id. Canonical targets by basis month are compared with prior ledger sums; immutable signed reallocation differences correct open periods and redirect frozen-month differences forward. Prefreeze rechecks targets; P100/C1, Sep30 refund30 and Oct1 refund30 arrivingOct2 yield Sep0/Oct−1 in both delivery orders. Semantic period snapshot hash excludes transport/correction history; audit_artifact_hash preserves that history. Both monthly and lifetime totals converge for the same known prefreeze facts.
...
| перестановка | НЕ ВЫПОЛНЕНА | Refund-before-payment, canceled-after-success, partial refund order меняют итог | Parent wait/reconcile, immutable succeeded facts, cumulative refund lock/formula | Planned: all permutations + full refund+close race; compare each month economic rows/hash as well as capped lifetime reversal; late earlier fact after freeze redirects differences, no rollback activation; fixture path not created |
...
Historical payout recovery is outside provider event intake: the same observation/evidence route requires approved pre-transfer balances/order separately from current counter inclusion/deltas, including NPD declaration coverage. It never treats current totals minus a transfer as historical tax evidence and never triggers a payout API.
