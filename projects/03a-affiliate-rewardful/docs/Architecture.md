...
Calendar policy from ADR-005: Sep payment received Oct2 can enter open September; after September freeze it posts as a late adjustment to next open current period, original instant preserved. Close never silently drops simultaneous posting; program lock resolves which side of freeze owns the event. Due date is the manual TRANSFER deadline on the 5th, unchanged on weekends. Ordinary preparation/confirm requires this date; an explicit owner-approved late catch-up preparation may use a current/future planned date with reason and tax/guard checks, while its actual late transfer still completes through observation/evidenced reconciliation with original due_date; actual early/late transfers use observation/reconciliation and expose the deviation without erasing facts or moving due_date. NPD receipt due after payment is a tracked document obligation, not impossible prepayment proof. Actual transfer contradicting tax preparation is preserved by the separate observation route even under closed RecoveryGate or without preparation. Late reporting is tested against actual transfer validity, not report time; evidence reconciliation computes historical tax from approved immediately-before-transfer balances/order, independently applies current inclusion deltas to guard total/NPD internal/NPD coverage/base/withheld, then updates old/actual guards and YTD atomically. Current totals (even minus observed gross) cannot reconstruct the historical tax basis; a current NPD declaration that already includes a missing internal payment requires coordinated internal+coverage deltas. History cannot pretend the transfer did not occur.
...
V3: перепрочитаны Data Structures и Core Algorithms после PSEUDO-01..06/CH-01..05 и V2-01, первоначальное утверждение о чистоте v1 заменено этой сверкой. Сверены сущности: User, EnrollmentGrant, Session, Membership, Program, PolicyVersion, Partner, Invitation, EligibilityFact, PartnerAsset, Attribution, Connection, N1Outbox, EventReceipt, ProviderProof, Payment, Refund, RefundAllocationRevision, LedgerEntry, AccountingPeriod, Registry, RegistryRow, Allocation, Carry, TaxProfile, TaxRule, TaxYTD, PayerYearGuard, TaxSnapshot, PayoutPreparation, PayoutConfirmation, ReconcileException, AuditEvent, GrowthEvent, Entitlement, RecoveryGate; typed values BusinessEvent, RegistrationEvent, ProviderVerification, TransferObservation, HistoricalTaxBasis, CurrentCounterPlan, CounterDecision. Все14 алгоритмов от AuthenticateSession до ReconcileAndRecover перечитаны вместе с отображением выше.

| Сущность.поле | Вид расхождения | Что сделано |
|---|---|---|
| TaxSnapshot.historical_basis/current_counter_plan | отсутствующая колонка/неверная временная база | Persisted typed approved pre-transfer balances/order separate from current per-counter inclusion/deltas; historical formula never reads inclusive/later current balances |
