// из N5: projects/05-podcast-clips-opus/scripts/responsive/input.mjs (коммит 90fe80a) — разбор аргументов, preflight и
// коды возврата без изменений; фикстура N6: почта и пароль тестового аккаунта, маршруты каркаса design-shell.
...
// Маршруты N6 на фиче design-shell: лендинг, тарифы, вход, пустой кабинет. /b/{slug}, предпросмотр и страница-хозяин
// с виджетом (http://localhost:8099) добавляются своими фичами — тогда фикстура получит их идентификаторы.
export const ROUTES = ['/', '/pricing', '/login', '/dashboard'];
export function validateFixture(f, base) {
  if (!f || ['email', 'password'].some(k => typeof f[k] !== 'string' || !f[k].trim())) throw new Error('Фикстура непригодна: нужны email и password');
  if (f.origin && f.origin !== base) throw new Error('Фикстура непригодна: origin');
  return { email: f.email, password: f.password, routes: [...ROUTES] };
}
