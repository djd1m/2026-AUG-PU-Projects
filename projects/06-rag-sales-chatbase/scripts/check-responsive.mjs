// из N5: projects/05-podcast-clips-opus/scripts/check-responsive.mjs (коммит 90fe80a) — адаптировано: вход через /login,
// cookie темы n6_theme, без предусловий клипов N5 (/c/, /g/, экран записи), CSS-линт только globals.css.
...
// Theme is an OUTER dimension (A-2509-03): dark = no cookie (checks the default), light = cookie n6_theme=light.
...
export async function themedContext(browser, options, theme, base) {
  ...
  if (theme === 'light') await context.addCookies([{ name: 'n6_theme', value: 'light', url: base }]);
  return context;
}
...
// Предусловия маршрутов: кабинет обязан открыться под входом (иначе прогон проверил бы форму входа).
export async function routePreconditions(page, route) {
  if (route.startsWith('/dashboard') && await page.locator('form.auth-card').count() > 0) throw new Error('Фикстура непригодна: кабинет открылся без сессии');
}
...
export async function main(args, { launchOptions = {} } = {}) {
  ...
  try {
    ...
    for (const file of ['apps/web/src/app/globals.css']) report.findings.push(...lintCSS(await readFile(file, 'utf8'), file));
    const playwright = await import('./responsive/playwright.mjs');
