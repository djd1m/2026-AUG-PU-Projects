// из N5: projects/05-podcast-clips-opus/scripts/check-env-wiring.mjs — сервисы web и worker-index, пакеты @n6/*, config в packages/rag
...
function resolveImport(file, name, projectRoot) {
  ...
  else if (name.startsWith('@n6/')) {
    const [pkg, ...rest] = name.slice('@n6/'.length).split('/');
...
export function collectWebConfigEnvironment(projectRoot) {
  const file = path.join(projectRoot, 'packages/rag/src/config.ts');
  const tree = ts.createSourceFile(file, fs.readFileSync(file, 'utf8'), ts.ScriptTarget.Latest, true);
