...
|---|---|---|---|
| SC-US-001-1 | краулинг | Specification.md:427 | ввод URL → 202 с index_job_id за ≤2с |
| SC-US-001-2 | краулинг/robots | Specification.md:428 | Disallow: / → отказ за ≤10с, предложение PDF |
| SC-US-001-3 | краулинг/JS-сайт | Specification.md:429 | пустая оболочка → `no_text`, не «готово, 0 страниц» |
| SC-US-001-4 | краулинг/SSRF | Specification.md:430 | 127.0.0.1 или частный адрес → `blocked_address`, 0 запросов |
| SC-US-002-1 | ответ | Specification.md:434 | ответ ≤6с p95 с цитатой и ссылкой |
| SC-US-002-2 | «не знаю» | Specification.md:435 | вопрос вне базы → отказ, модель ответа не вызвана |
| SC-US-002-3 | лимит предпросмотра | Specification.md:436 | 11-й вопрос в предпросмотре → отказ с CTA регистрации |
| SC-US-003-1 | claim | Specification.md:440 | сохранение бота при регистрации без повторного чтения |
| SC-US-003-2 | claim/срок | Specification.md:441 | токен >24ч → регистрация проходит, бот не восстановлен |
| SC-US-003-3 | claim/чужой | Specification.md:442 | чужой/использованный токен → 404/409 |
| SC-US-004-1 | PDF | Specification.md:446 | текстовый PDF → индексация, цитата «файл.pdf, с. N» |
| SC-US-004-2 | PDF/скан | Specification.md:447 | скан без текстового слоя → `no_text_layer` |
| SC-US-004-3 | PDF/предел | Specification.md:448 | >10 МБ или 4-й PDF на free → отказ до загрузки |
| SC-US-005-1 | установка | Specification.md:452 | тег + список CSP-директив на экране установки |
| SC-US-005-2 | домены | Specification.md:453 | добавление домена → origin в списке |
| SC-US-005-3 | контакт обязателен | Specification.md:454 | нет контакта → код не показан |
| SC-US-006-1 | ответ посетителю | Specification.md:458 | плашка «Источник: … ↗» |
| SC-US-006-2 | «не знаю» посетителю | Specification.md:459 | отказ с контактом, `unknown` в журнале |
| SC-US-006-3 | prompt injection | Specification.md:460 | вредоносная страница не меняет поведения бота |
| SC-US-006-4 | выдуманная цитата | Specification.md:461 | метка вне контекста → «не знаю», не текст |
| SC-US-007-1 | лимит посетителя | Specification.md:465 | 21-й вопрос сессии → `refused_limit`, без вызова модели |
| SC-US-007-2 | месячный потолок | Specification.md:466 | исчерпан → отказ посетителю + баннер владельцу |
| SC-US-007-3 | конкурентность лимита | Specification.md:467 | 20 одновременных при остатке 1 → ровно 1 ответ |
| SC-US-008-1 | CORS/домен вне списка | Specification.md:471 | evil.example → 403 без ACAO, без квоты |
| SC-US-008-2 | CORS/домен в списке | Specification.md:472 | ровно один заголовок ACAO |
| SC-US-009-1 | установка засчитана | Specification.md:476 | первый ответ на домене → «подключён» |
| SC-US-009-2 | демо ≠ установка | Specification.md:477 | запросы с N6_PUBLIC_ORIGIN не считаются |
| SC-US-010-1 | сводка | Specification.md:481 | «ответил / не знал» + список unknown |
| SC-US-010-2 | сводка/пусто | Specification.md:482 | «вопросов ещё не было», без % (CFG-I7) |
| SC-US-011-1 | экран интереса | Specification.md:486 | «Убрать бейдж» → interest, без платёжных полей |
| SC-US-011-2 | бейдж снят | Specification.md:487 | план nobadge → `badge_required=false` |
| SC-US-012-1 | перенос бота | Specification.md:491 | «Передать клиенту» → приглашение на 7 дней |
| SC-US-012-2 | приём приглашения | Specification.md:492 | клиент — владелец, студия — «только чтение» |
| SC-US-012-3 | предел ботов studio | Specification.md:493 | 11-й бот → отказ с названием предела |
| SC-US-013-1 | демо-страница | Specification.md:497 | `/b/{slug}` без входа с бейджем |
| SC-US-013-2 | noindex | Specification.md:498 | индексация не включена → `noindex` |
| SC-US-013-3 | снятие публикации | Specification.md:499 | снята → 404 |
| SC-US-014-1 | переиндексация | Specification.md:503 | только изменённые страницы переэмбеддятся |
| SC-US-014-2 | удаление источника | Specification.md:504 | ответ не ссылается на удалённые фрагменты |
| SC-US-015-1 | удаление аккаунта | Specification.md:508 | виджеты 403 сразу, данные стёрты ≤72ч |
| SC-US-016-1 | сбой индексации | Specification.md:512 | шлюз недоступен → `failed`, кнопка «Повторить» |
| SC-US-016-2 | продолжение повтора | Specification.md:513 | эмбеддятся только оставшиеся страницы |
| SC-US-016-3 | сторож | Specification.md:514 | мёртвый воркер → `failed(stalled)` ≤6 мин |
