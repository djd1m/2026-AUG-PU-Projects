// из N5: projects/05-podcast-clips-opus/tests/auth.test.ts — cookie __Host-n6_session, форма ошибок N6, XFF из одного адреса, IPv6 /48
...
describe('Аутентификация', () => {
  it('Нет адреса / неверный пароль / erasing: одинаковые текст и статус, сопоставимое время', async () => {
    ...
    expect(replies[0]).toEqual({ status: 401, body: { error: { code: 'invalid', message: 'Неверная почта или пароль' } } });
  ...
  it('Регистрация не сообщает о занятом адресе и считает bcrypt до записи', async () => {
    ...
    expect(result.status).toBe(200); expect(await result.json()).toEqual({ data: { ok: true } });
  });
  it('logout сначала отзывает серверную сессию, затем удаляет cookie', async () => {
    ...
    const result = await handler('logout', db)(request(undefined, { cookie: `__Host-n6_session=${token}` }));
  ...
  it('IP: берёт единственный адрес двери, сохраняет только /24 или /48', () => {
    expect(clientIp(new Headers({ 'x-forwarded-for': '192.0.2.99' }))).toBe('192.0.2.99');
    expect(ipPrefix('::ffff:192.0.2.99')).toBe('192.0.2.0/24');
    expect(ipPrefix('2001:db8:1234::1')).toBe('2001:db8:1234::/48');
