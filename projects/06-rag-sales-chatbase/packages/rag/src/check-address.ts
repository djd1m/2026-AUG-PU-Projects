...
// reuse-inventory §9). Живёт в @n6/rag с фичи preview-flow: его зовут И web (POST /api/preview — отказ ДО
// постановки и ДО квоты), И worker-index (краулер — на каждом соединении и перенаправлении). Подпуть
// '@n6/rag/check-address' — не из index: клиентские компоненты web не тянут node:dns. Краулер не сканер нашей сети: схема, учётные данные и порт проверяются ДО DNS;
