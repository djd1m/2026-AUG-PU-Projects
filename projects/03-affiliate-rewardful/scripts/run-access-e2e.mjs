...
const directory=resolve('.runtime/access-e2e'),container='n3-access-browser';
const driverUrl='http://127.0.0.1:4572',sessionPath=directory+'/webdriver-session.json';
...
try {
  for(const port of [13145,13146,4572])await free(port);
  execFileSync('openssl',['req','-x509','-newkey','rsa:2048','-nodes','-keyout',directory+'/key.pem','-out',directory+'/cert.pem','-days','2','-subj','/CN=oauth.yandex.ru','-addext','subjectAltName=DNS:oauth.yandex.ru,DNS:*.212.192.0.33.sslip.io'],{stdio:'ignore'});
  execFileSync('docker',['compose','-f','docker-compose.test.yml','run','--rm','--no-deps','-d','--name',container,
    '-p','127.0.0.1:13145:13145','-p','127.0.0.1:13146:13146','backend','node','tests/helpers/access-browser-server.mjs'],{stdio:'pipe'});started=true;
  execFileSync('docker',['network','connect','n3-frontend',container],{stdio:'pipe'});
  await ready('http://127.0.0.1:13146/context');
  driver=spawn('geckodriver',['--port','4572','--host','127.0.0.1','--profile-root',process.env.N3_FIREFOX_PROFILE_ROOT || '/root/snap/firefox/common'],{stdio:['ignore','pipe','pipe']});
  ...
  const response=await fetch(driverUrl+'/session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({capabilities:{alwaysMatch:{browserName:'firefox',acceptInsecureCerts:true,
    proxy:{proxyType:'manual',httpProxy:'127.0.0.1:13145',sslProxy:'127.0.0.1:13145',noProxy:[]},
  ...
  const child=spawn(process.execPath,['--test','--test-concurrency=1','tests/e2e/access.mjs'],{env:{...process.env,N3_WEBDRIVER_URL:driverUrl,N3_WEBDRIVER_SESSION:sessionPath},stdio:['ignore','pipe','pipe']});
