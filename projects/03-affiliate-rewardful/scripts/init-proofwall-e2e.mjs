...
import { N3 } from '../tests/helpers/proofwall-bridge/tls.mjs';
...
function certificate(dir,hosts) {
  if(existsSync(`${dir}/cert.pem`)) {
    const text=execFileSync('openssl',['x509','-in',`${dir}/cert.pem`,'-noout','-ext','subjectAltName'],{encoding:'utf8'});
    if(hosts.every(host=>text.includes(`DNS:${host}`)))return;
  }
...
certificate(`${root}/n3`,['a','b','c','d'].map(v=>`n3-${v}.212.192.0.33.sslip.io`));
...
if(process.argv.includes('--bind')) {
  ...
  if(bootstrap.n3Origin!==N3)throw new Error('Bootstrap variant mismatch');
  const lines=readFileSync(base,'utf8').split('\n').filter(l=>l && !/^N3_(TENANT_ID|CONNECTOR_KEY|BASE_URL)=/.test(l));
