...
async function refreshFulfillment(invoice) {
  ...
  const fullyFunded = order.verified === true
    ...
    && order.netAmountMinor === invoice.amountMinor;
...
Пример консервативен: боевой доступ требует нетестового платежа на полную сумму доверенного счёта без возвратов. Частичный возврат тоже закрывает доступ в этом примере; иной вариант (пропорциональное уменьшение срока или ручная сверка) должен быть явно задан правилами вашего продукта. Полностью возвращённый заказ остаётся исторически `succeeded`, но имеет `netAmountMinor: 0`. Для тестового магазина проверяйте выдачу только изолированного тестового доступа; никогда не включайте его через снятие проверки `testMode` в production. Повторное чтение статуса безопасно.

Публичный HTTP-ответ при недоступном или ненастроенном провайдере — `503` с `error.code: "UNAVAILABLE"`; этот код сам по себе не различает отсутствие настройки и сбой провайдера. Конфигурацию проверяет владелец в статусе оплаты кабинета. Покажите недоступность и повторите позже, не создавайте локальный «успех».
