...
```
projects/<NN-название>/
├── README.md          # что за продукт, простыми словами + для специалистов
├── CLAUDE.md          # контекст проекта (генерируется Phase 3)
├── decisions/         # журнал развилок — ЛОКАЛЬНЫЙ для проекта
├── docs/              # SPARC-документация + discovery + validation
└── .claude/           # тулкит проекта: агенты, правила, roadmap
```

**`decisions/` заводится в каждом проекте отдельно.** Развилка «сколько сайтов считать успехом
недели» относится к Senja-клону и бессмысленна для клона Cal AI. Общие методические решения
живут в [`/start/`](../start/), дефекты пайплайна — в [`/harness-forge/`](../harness-forge/).
...
## Как читать телеметрию работы

Общие инструкции для всех проектов:

- [Для человека](TELEMETRY-HUMAN.md): порядок чтения, результаты проверок, время,
  модели, расход и примеры из репозитория.
- [Для ИИ-агента](TELEMETRY-AGENT.md): проверка доказательств, нормализация разных
  форматов, защита от двойного счёта и шаблон аналитического отчёта.

Сами записи остаются внутри проектов: `docs/telemetry/p-replicator/<RUN_ID>/`.

Для практического анализа есть общий skill
[project-telemetry](../.claude/skills/project-telemetry/SKILL.md): диагностика причин
задержек, прогноз по сопоставимым задачам и оценка изменений пайплайна.
В Codex или Claude Code попросите прочитать этот `SKILL.md` и указать выбранный проект.
