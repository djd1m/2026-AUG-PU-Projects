...
INPUT: `bucket` (`register`|`login_fail`|`public_code`|`api_write`), `key`, `limit`, `window_sec`
...
REALISES: SC-US-001-1, SC-US-001-2, SC-US-009-1, SC-US-009-2, SC-US-009-3
...
2. Валидация: `email ← lower(trim(email))`, формат `local@domain.tld`, ≤ 254; `len(password) ≥ 10`, ≤ 128; `consent_pd === true` AND `consent_terms === true`; иначе `422` с перечнем полей.
...
STEPS:
1. **login.** `email ← lower(trim(email))`. `rate_limit('login_fail', email, 10, 900)` читается ДО проверки пароля: IF текущий счётчик окна ≥ 10 THEN RETURN 429 (проверка без инкремента: `GET k`).
2. `acc ← SELECT id, password_hash FROM account WHERE email = $email` (без транзакции).
3. `ok ← acc ? argon2.verify(acc.password_hash, password) : argon2.verify(DUMMY_HASH, password) && false` — время ответа одинаково для существующего и нет.
4. IF NOT ok THEN `INCR` счётчика `login_fail:{email}` (как в «Лимит частоты», шаг 2); RETURN 401 «неверный email или пароль».
5. `access ← JWT HS256 {sub: account_id, exp: now+15 мин}` на `JWT_SECRET`. `refresh ← base64url(random(32))`; `INSERT refresh_token(account_id, token_hash=sha256(refresh), expires_at=now()+30 дней [ПРЕДЛОЖЕНИЕ])`.
6. Cookie `refresh`: `HttpOnly; Secure; SameSite=Lax; Path=/api/auth; Max-Age=2592000`. RETURN 200.
7. **refresh.** Транзакция: `UPDATE refresh_token SET revoked_at = now() WHERE token_hash = sha256($c) AND revoked_at IS NULL AND expires_at > now() RETURNING account_id`. Нет строки → IF токен существует с `revoked_at IS NOT NULL` THEN отозвать все refresh этого аккаунта (повторное использование = кража); RETURN 401. Иначе выдать новую пару как в шаге 5–6.
8. **logout.** `UPDATE refresh_token SET revoked_at = now() WHERE token_hash = sha256($c)`; стереть cookie; RETURN 200.
COMPLEXITY: O(1).
...
REALISES: SC-US-006-3, SC-US-010-3, SC-US-012-4
INPUT: запрос, требования `{verified?: bool, role?: 'operator', owns?: {table, id}}`
OUTPUT: `ctx = {account_id, plan, role}` | `401` | `403` | `404`
STEPS:
1. Middleware: удалить из входящего запроса все заголовки `x-user-*`; проверить JWT (подпись, `exp`); нет или невалиден → 401 (для страниц — редирект на вход).
...
3. **Уборщик** — в `worker-ai`, раз в 60 с, только держатель `pg_try_advisory_lock(LEASE_SWEEPER_LOCK)`:
   ...
   2. FOR EACH `j`: `expected ← expected_bullmq_ids(j)` (шаг 4). `states ← BullMQ.getState(id)` для каждого.
      ...
      - После COMMIT: для каждого `id` из `expected` в состоянии `completed` | `failed` → `Job.remove(id)`; затем `add` с тем же `jobId`. Работа в состоянии `active` не трогается (её снимет механизм stalled BullMQ по `lockDuration` 120 с).
...
5. `finish_failed(tx, j, reason)`: `UPDATE job SET status='failed', fail_reason=$reason, finished_at=now() WHERE id=$j AND status='running' RETURNING id`; IF обновилось THEN `INSERT event(name='job_failed', account_id, props={job_id, reason, step})`; журнал JSON `{job_id, step, reason}`.
...
6. `execFile('ffmpeg', ['-i', src, '-vn', '-ac','1', '-ar','16000', '-c:a','libmp3lame', '-b:a','64k', audio])`.
...
8. **План кусков** (детерминирован для одного и того же аудио):
   ...
   ```
   Каждый кусок: `execFile('ffmpeg', ['-i', audio, '-ss', sec(offset), '-t', sec(dur), '-c:a','libmp3lame','-b:a','64k', out])` (перекодирование: смещение точное); `PutObject('tmp/{job_id}/chunk-{k}.mp3')`.
...
10. `admit_stt(job_id)` (следующий алгоритм); отказ → RETURN (задача уже `failed`).
...
REQUIREMENT: `AC-clips-12`
REQUIREMENT: `NFR-clips-2`
REALISES: SC-US-002-4, SC-US-002-5
INPUT: `job_id`
OUTPUT: `ok` | задача `failed/quota_user` | `failed/quota_global`
STEPS:
1. `jday ← msk_day(job.created_at)` — день задачи, неизменный для неё.
2. Транзакция:
   1. `SELECT 1 FROM quota_counter WHERE scope='job' AND scope_id=$j AND day=$jday AND kind='stt_sec'`. IF есть THEN COMMIT; RETURN ok (допуск уже был; повтор подготовки не резервирует повторно).
   2. `need ← Σ ceil(duration_ms / 1000)` по кускам `status='pending' AND attempt_count=0` (секунды, которые реально уйдут провайдеру, с перекрытиями).
   3. `r ← reserve([{account, account_id, today, 'stt_sec', need, LIMIT_STT_USER_SEC_DAY, 'quota_user'}, {global, GLOBAL_SCOPE_ID, today, 'stt_sec', need, LIMIT_STT_GLOBAL_SEC_DAY, 'quota_global'}, {job, job_id, jday, 'stt_sec', need, need, '—'}])` — строка `job` служит отметкой «допуск сделан».
   4. IF NOT r.ok THEN ROLLBACK; новая транзакция `finish_failed(j, r.fail_reason)`; RETURN (вызова STT нет; `spend_ledger` не меняется — AC-clips-12).
3. RETURN ok. Первая попытка каждого куска оплачена этим допуском; каждая следующая попытка резервирует секунды куска сама («Транскрипция куска», шаг 3).
COMPLEXITY: O(c).
...
3. IF `n ≥ 2` THEN `r ← reserve([{account,…,'stt_sec', ceil(dur/1000), LIMIT_STT_USER_SEC_DAY,'quota_user'}, {global,…}])`; отказ → ROLLBACK; `finish_failed(r.fail_reason)`; RETURN. (Каждая попытка учтена ДО вызова; повтор расходует потолок как успех.)
4. `INSERT spend_ledger(account_id, job_id, call='stt', model=STT_MODEL, attempt=n, units_reserved=ceil(dur/1000), cost_kop=0, outcome='timeout') RETURNING id` — пессимистичная запись: падение процесса посреди вызова читается как «таймаут, резерв удержан». COMMIT.
...
7. Разбор результата — алгоритм «Разбор ответа STT». Исход:
   - `ok(units, cost_usd_micro?)`: транзакция `UPDATE spend_ledger SET outcome='ok', units_actual=ceil(dur/1000), cost_usd_micro, cost_kop=ceil(cost_usd_micro × FX_USD_RUB_KOP / 1e6) WHERE id`; `UPDATE transcript_chunk SET status='done', units=$units, model=resp.model WHERE id AND status='pending'`.
   ...
   - сетевой отказ, 5xx, 429: `outcome = timeout ? 'timeout' : 'provider_error'`; бросить исключение → повтор BullMQ (3 попытки, экспонента 5 с). После последней попытки обработчик `failed` BullMQ вызывает `finish_failed('stt_failed')`. Подмены модели нет (ADR-004).
...
3. `in_tokens ← ceil(chars(prompt) / 3) + 500`; `reserve_usd_micro ← in_tokens × price_in + LLM_MAX_OUTPUT_TOKENS × price_out` (`MODEL_PRICES[LLM_MODEL]`); `reserve_kop ← ceil(reserve_usd_micro × FX_USD_RUB_KOP / 1 000 000)`.
...
5. Вне транзакции: `POST https://openrouter.ai/api/v1/chat/completions` `{model: LLM_MODEL, messages, max_tokens: LLM_MAX_OUTPUT_TOKENS, temperature: 0.2, response_format: {type:'json_schema', json_schema:{name:'clip_selection', strict:true, schema: SELECTION_SCHEMA}}, usage:{include:true}}`, без поля `models`, таймаут `LLM_HTTP_TIMEOUT_MS`.
   `SELECTION_SCHEMA = {fragments: array(maxItems 10) of {start_unit:int, end_unit:int, title:string(1..100), hook_quote:string, hook_reason:string, hook_score:int 0..10, completeness_quote:string, completeness_reason:string, completeness_score:int 0..10}}`, все поля обязательны, лишние запрещены.
...
7. Разбор: `JSON.parse` + проверка схемы кодом (даже при `strict`); плюс `0 ≤ start_unit ≤ end_unit ≤ N−1`. Нарушение → `outcome='schema_invalid'`; бросить `SchemaInvalid` → повтор BullMQ (попыток `LIMIT_LLM_ATTEMPTS_JOB`); вторая неудача упрётся в счётчик `llm_attempts` на шаге 4 → `failed/selection_failed` (AC-clips-8). 5xx/429/сеть → `outcome='provider_error'|'timeout'`, повтор тем же путём. Подставных моментов без LLM нет.
...
OUTPUT: строка фильтра видео и геометрия плашки знака
STEPS:
1. IF `w > h` THEN `vf ← "scale=1080:608:force_original_aspect_ratio=decrease,pad=1080:1920:(1080-iw)/2:420+(608-ih)/2:black"` — кадр вписан в полосу y = 420…1028 целиком, поля чёрные (ADR-009). ELSE `vf ← "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:black"`. Кропа нет.
...
3. Плашка: левый верхний угол (70, 200), размер `pw × ph`, радиус 12, цвет `&H1A000000` (альфа 0,9), текст белый полужирный `fs`, отступ 16×10. Контраст белого на почти чёрном ≥ 4,5:1.
...
STEPS:
1. `esc(t) ← t.replace(/\\/g,'＼').replace(/\{/g,'\\{').replace(/\}/g,'\\}').replace(/[\r\n]+/g,' ')` — пользовательский текст не может открыть блок переопределения или вставить `\N` (донор `escapeAssText` + обратная косая).
...
7. IF знак требуется THEN слой 0: событие-рисунок `{\an7\pos(70,200)\p1}` со скруглённым прямоугольником `pw × ph` радиуса 12 (8 кривых Безье) на всю длину клипа; слой 1: `{\an7\pos(86,210)\fs{fs}\b1}` + `esc(WATERMARK_TEXT)`. Знак и субтитры идут одним фильтром `ass=` — вшиты в пиксели.
...
5. `failed`: `v.retryable ← fail_reason ∉ {file_invalid, duration_exceeded} AND video.source_deleted_at IS NULL`; для `quota_*` — `v.resets_at` (00:00 МСК).
...
2. IF `render_status ≠ 'ready'` THEN RETURN 404.
...
5. `url_normalized ← 'https://' + host + u.pathname.replace(/\/+$/, '')` — запрос и фрагмент отбрасываются, регистр пути сохраняется.
...
3. `confirm(id, checks)`: `checks` обязан содержать все четыре отметки `public`, `this_clip`, `watermark_visible`, `account_not_empty` (иначе 422). Транзакция: `UPDATE publication SET status='confirmed', verified_at=now() WHERE id AND status='candidate' RETURNING clip_id, account_id`; нет строки → 409. `record_event('publication_confirmed', account_id, clip_id)`; `INSERT audit_log(actor=operator_email, action='publication.confirm', target=id, reason=checks)`.
...
### Algorithm: Заведение партнёра и выдача кода

REQUIREMENT: `FR-GROWTH-004`
REQUIREMENT: `FR-clips-11`
REALISES: SC-US-010-1, SC-US-010-2
INPUT: `/admin/partners`, процедура `create {name, contact, audience_url, code?, account_email?}`
...
REQUIREMENT: `FR-clips-11`
REQUIREMENT: `FR-clips-1`
INPUT: `/admin/users` процедуры `set_plan {email, plan, reason}`, `reset_password {email, reason}`; CLI `ops grant-operator <email>`
...
2. `GET /c/{clip_code}`: `rate_limit('public_code', ip, 30, 3600)`; IF не `/^[a-z0-9]{6,8}$/` OR клипа нет THEN `302 /` без события.
...
3. `confirmed ← publication` со `status='confirmed'` (на момент запроса; перепроверка 7-го дня уже перевела удалённые в `removed`), чей клип принадлежит задаче, созданной в `W`.
   `clips_confirmed ← COUNT(DISTINCT clip_id)`; `authors_confirmed ← COUNT(DISTINCT account_id)`; `goal_met ← clips_confirmed ≥ 5 AND authors_confirmed ≥ 3`. Несколько ссылок на клип — 1 клип.
...
### Algorithm: Расход на модели за сутки
...
INPUT: `GET /admin/spend`
OUTPUT: расход за сегодня (МСК) до счёта провайдера
STEPS:
1. `auth({role:'operator'})` — без роли 404.
...
3. Из `spend_ledger` за `d`: число попыток по `call` и `outcome`; сумма `cost_kop` (факт или удержанный резерв) — рублёвая оценка.
...
REQUIREMENT: `FR-clips-13`
REQUIREMENT: `NFR-clips-6`
INPUT: таймер в `worker-ai`, раз в 15 мин, держатель `pg_try_advisory_lock(STORAGE_SWEEPER_LOCK)`
...
5. Страховка вне кода (`config-only`): правило бакета — исходники 7 дней, `tmp/` 1 день, клипы 30 дней (ADR-007).
...
Все ответы JSON `{data, meta}` при 2xx и `{error: {code, message}}` при 4xx/5xx. `Authorization: Bearer <access JWT>`
у всех маршрутов с доступом «сессия»/«владелец»/`operator`; `refresh` — в cookie. Страницы `/admin/*` отдают HTML,
...
| `GET /p/{partner_code}` | — | `302 /?via=p` (± `Set-Cookie pref`) | `429` |
| `GET /c/{clip_code}` | — | `302 /?via=c` | `429` |
...
Scenarios in Specification.md: 48  ·  claimed by an algorithm: 46
