...
[canon.md](canon.md) (хеш `73fce1cb…0d4` на момент сдачи), [decisions-owner.md](decisions-owner.md).
...
| `LLM_MAX_OUTPUT_TOKENS` | 3 000 `[ПРЕДЛОЖЕНИЕ; выход замеряется по `usage.completion_tokens` в `spend_ledger.units_actual` на первых 3 реальных выпусках — проба STT дня 1 шага LLM не содержит (V3-11 б)]` | ADR-005, VA2-04 |
...
| `LLM_EST_CHARS_PER_SEC` | переменная окружения `worker-ai` без дефолта (канон §6), `22`; калибруется по первым 3 выпускам | OWN-05A-012, канон §6 |
...
3. `errors ← []`. FOR EACH `name` IN `required`:
   ...
   3. `VALIDATOR[name](raw)`; ошибка → `errors.push(name, 'невалидно: <причина>', …)`. Валидаторы:
      ...
      - `LIMIT_*`, `FX_USD_RUB_KOP`, `LLM_EST_CHARS_PER_SEC`: `/^[1-9][0-9]*$/`, целое > 0 (число ≤ 0 и `'0'` — ошибка).
...
5. `day` для ВСЕХ счётчиков, включая `scope='job'`, — `msk_day(now())` на сервере БД в момент попытки (канон §11, VT2-11); `job.created_at` день не задаёт. Отметки допуска ищутся без условия на `day` (их не больше одной на задачу).
...
REQUIREMENT: `AC-clips-27`
REALISES: SC-US-003-3, SC-US-003-4, SC-US-003-5, SC-US-003-6
...
3. IF `n ≥ 2` THEN `r ← reserve([{global, GLOBAL_SCOPE_ID, d, 'stt_sec', provider_sec(k), LIMIT_STT_GLOBAL_SEC_DAY, 'quota_global'}] + (есть отметка новичка задачи `('job', $j, *, 'stt_sec_newbie')` ? [{global, GLOBAL_SCOPE_ID, d, 'stt_sec_newbie', provider_sec(k), LIMIT_STT_NEWBIE_GLOBAL_SEC_DAY, 'quota_global'}] : []))`; отказ → ROLLBACK; `finish_failed('quota_global')`; RETURN. **Персональный `stt_sec` повтор НЕ списывает** (V3-10, канон §11): допуск уже списал с автора всю длительность записи `ceil(Σ unique_ms / 1000)` — для 120 мин весь `LIMIT_STT_USER_SEC_DAY`, и первый же сетевой повтор давал бы ложный `quota_user` посреди оплаченной задачи. Повтор — это секунды провайдера: они идут в глобальный счётчик (и в пул новичков, если задача допущена как новичок). Тест: выпуск 120 мин, 5xx на куске 3 → повтор проходит, персональный счётчик = 7 200, глобальный вырос на `provider_sec(3)`.
...
7. **Сначала — обрыв по длине, ДО `JSON.parse`** (VA2-04, V3-11): IF `resp.choices[0].finish_reason == 'length'` THEN `outcome='schema_invalid'`, журнал `llm_output_truncated {completion_tokens}`, `finish_failed('selection_failed')` сразу и `UnrecoverableError` для BullMQ — БЕЗ повтора: повтор с тем же `max_tokens` оборвётся так же, а обрезанный JSON иначе бросил бы ошибку разбора и ушёл бы в обычный повтор. Тест: ответ с `finish_reason='length'` и обрезанным JSON → одна попытка в `spend_ledger`, задача `selection_failed`.
   Затем разбор: `JSON.parse` + проверка схемы кодом (даже при `strict`): типы, `len(fragments) ≤ 10`, `0 ≤ hook_score, completeness_score ≤ 10`, `1 ≤ len(title)`, `0 ≤ start_unit ≤ end_unit ≤ N−1`. Поле длиннее `LLM_FIELD_MAX_CHARS` обрезается кодом по границе слова с «…» (цитата после обрезки — префикс исходной и проверяется как обычно); это не ошибка схемы, деньги на повтор не тратятся. Нарушение схемы (при `finish_reason ≠ 'length'`) → `outcome='schema_invalid'`; бросить `SchemaInvalid` → повтор BullMQ (попыток `LIMIT_LLM_ATTEMPTS_JOB`); вторая неудача упрётся в счётчик `llm_attempts` на шаге 4 → `failed/selection_failed` (AC-clips-8). 5xx/429/сеть → `outcome='provider_error'|'timeout'`, повтор тем же путём. Подставных моментов без LLM нет.
...
2. **Геометрия знака — считается один раз при старте `worker-render`:** для `fs` от 40 до 52 (канон §11): `tw ← ширина WATERMARK_TEXT` по метрикам шрифта (сумма advance × fs / unitsPerEm), `pw ← tw + 32`, `ph ← ceil(fs × 1,2) + 20`, `area ← pw × ph / (1080 × 1920)`. Взять НАИБОЛЬШИЙ `fs` с `0,01 ≤ area ≤ 0,04` И `pw ≤ 342 − 2 × WM_INSET = 294` (крупнее — читаемее после сжатия площадкой) (V3-12: плашка обязана уместиться в полосу 342 px, остающуюся от кадра 16:9 при кадрировании площадкой на весь экран 9:16 — `crop=342:608:369:420`). IF такого нет THEN завершить процесс: «WATERMARK_TEXT: знак не укладывается в 1–4 % кадра и в 294 px ширины при кегле 40–52 — геометрия FR-GROWTH-003 п. 5 невыполнима». Шрифт — один файл `WM_FONT_FILE = /app/fonts/<файл>.ttf` в образе; его имя семейства читается из того же файла, передаётся в ASS (`Fontname`) и в фильтр как `ass=<файл>:fontsdir=/app/fonts` — libass не подставит запасной шрифт через fontconfig (VA2-10). **Самопроверка при старте `worker-render`:** отрендерить 1 кадр 1080×1920 белого фона со знаком; IF внутри прямоугольника плашки нет пикселей текста (яркость ≥ 240 при плашке ≤ 140) THEN процесс не стартует: «знак не рисуется: шрифт не найден libass».
3. **Знак по центру, у нижней кромки картинки видео, над субтитрами** (OWN-05A-015, заменяет угол OWN-05A-013; VA2-02: угол срезается кадрированием по спикеру, центр полосы — нет):
   ...
   Числа Specification FR-GROWTH-003 п. 4 и SC-US-008-1 (дословно): 16:9 и 4:3 — центр плашки x = 540, **нижний край плашки y = 1 004**; вертикальный исходник (в том числе видео с телефона с `rotate=90` после `display_dims`) — центр x = 540, **нижний край y = 1 084** (над субтитрами с y = 1 100). Формула выше даёт ровно их: 16:9/4:3 — `min(1 028 − 24, 1 084) = 1 004`, вертикальный — `min(1 920 − 24, 1 084) = 1 084`; при `ph = 78` (для примера кегль 48; реальный кегль — наибольший по правилу шага 2) верх плашки 926 и 1 006. Прочие пропорции (формула, не число Specification): 21:9 (1 080×463, y 492…955) — низ 931; квадрат 1:1 (y 420…1 500) — низ 1 084; крайне широкий 4:1 (1 080×270, y 589…859) — низ 835, верх 757 ≥ 613, помещается; полоса выше ~126 px или уже `pw + 48` → `watermark_fits = false`. Тест SC-US-008-1: центр плашки 540 ±2 px и нижний край 1 004 ±2 px для 16:9 и 4:3, 1 084 ±2 px для вертикального.
   IF NOT `watermark_fits` THEN подготовка отказывает файл `file_invalid` ещё до оплаты STT (VA2-09), а рендер при невозможном случае отказывает `render_failed` с журналом `watermark_no_room` — знак не кладётся на чёрное поле ни при каком исходнике. Плашка `pw × ph`, радиус 12, чёрная с непрозрачностью `WM_OPACITY` (ASS `&H73000000`: альфа 0x73 ≈ 0,55), текст белый полужирный `fs`, отступ 16×10. Контраст белого на плашке 0,55 поверх белого кадра ≈ 4,7:1 — у нижней границы 4,5:1, поэтому проверяется OCR на светлом, тёмном и пёстром кадре (SC-US-008-1), а не выводится. Тест мутацией: вернуть угол → проверка «плашка по центру полосы ±2 px и над y = 1 100» красная.
...
REQUIREMENT: `FR-clips-16`
REALISES: SC-US-001-6, SC-US-001-7, SC-US-001-8, SC-US-002-9
INPUT: `/admin/users` процедуры `set_plan {email, plan, reason}`, `reset_password {email, reason}`, `verify_email {email, reason}`; CLI `ops grant-operator <email>`, `ops beta-add <email>`, `ops reset-link <email>`
...
5. `ops reset-link <email>` — **неделя 1** (канон §7, VA2-22; страница `/admin/users` — вторая очередь): аккаунт по `canonical_email` (нет → код 1 и сообщение «аккаунт не найден», письмо не отправляется); далее шаг 2 без проверки роли и без `reason` из формы: `reason='ops-cli'`; письмо со ссылкой `{BASE_URL}/reset?token=` уходит на почту аккаунта, ссылка в stdout НЕ печатается (оператор токена не видит); `GET /reset` и `POST /api/auth/reset` — неделя 1, как у шага 2. `audit_log(actor='ops-cli', action='account.reset_link', target=email)`.
COMPLEXITY: O(1).
...
| `POST /api/videos/{video_id}/complete` | `Idempotency-Key`; `{parts:[{part_number, etag}]}` (ETag читается браузером благодаря `ExposeHeaders: ETag` CORS бакета; `PUT` частей шлёт только заголовок `Content-Type`, разрешённый `AllowedHeaders = content-type`, канон §8) | `202 {job_id}` | `404` · `409 upload_not_found` · `422 file_invalid` · `422 bad_idempotency_key` |
...
Scenarios in Specification.md: 69  ·  claimed by an algorithm: 67
