...
| Конкретная модель OpenRouter, которая возвращает метки спикеров на русской речи с нужной точностью | OpenRouter: кандидаты `assemblyai/universal-3-5-pro`, `deepgram/nova-3`, `microsoft/mai-transcribe-2`, `meta/muse-voice-transcribe-1.0`, `x-ai/grok-stt-1.0` | цитаты документации, что выбранная модель через OpenRouter возвращает `speaker` для русского, нет; ответ даёт только проба дня 1 (ADR-001, критерии FR-clips-4 п. 9) · checked 2026-09-23 | UNCONFIRMED | FR-clips-7 п. подписи спикеров, AC-clips-24 |
