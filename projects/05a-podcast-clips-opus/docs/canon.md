...
| `publication` | `id`, `clip_id`, `account_id`, `url`, `url_normalized` (уникальный), `platform`, `channel_key` (канал/профиль автора на площадке, нижний регистр; для подсчёта «разных авторов», VA-14), `status`, `reason`, `verified_at`, `rechecked_at`, `created_at`; при удалении клипа или видео `clip_id` становится `NULL`, подтверждённая публикация сохраняется для метрики (FR-clips-13 п. 2, FR-GROWTH-005 п. 4; В-26) |
