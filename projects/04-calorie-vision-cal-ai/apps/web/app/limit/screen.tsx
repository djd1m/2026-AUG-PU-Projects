...
//
// ИЗМЕНЕНО 2026-09-16 (OWN-001, фича `subscription-and-commission`). Прежняя редакция
// утверждала «оплаты сейчас нет» и не имела НИ ОДНОГО платёжного элемента — это кодировало
// решение PD-PRICE-001, которое владелец отменил. Теперь на экране есть ССЫЛКА на Pro, но
// НЕ форма оплаты: оплата начинается на отдельной странице, где названы цена и условия.
//
// Что сохранено из прежнего запрета и почему: бесплатный путь называется ПЕРВЫМ, до любого
// упоминания платного. Исходная причина запрета — продукт-источник был наказан магазином
// приложений за практики платной страницы — никуда не делась; изменилось лишь то, что
// продавать теперь можно. Проверяет `tests/integration/limit-screen-free-path-first.test.tsx`.
//
// Форма интереса оставлена НАМЕРЕННО: пока приём денег идёт через фейк (ИП и ключей нет),
// она единственная работает по-настоящему. Снимается вместе с переходом в живой режим —
// это решение владельца (OWN-001), а не моё.
  ...
  return (
    <main className="page" aria-label="экран лимита">
      <div className="card">
        ...
        {/* ИСПРАВЛЕНО (OWN-001): строка «оплаты сейчас нет» стала неверной — подписка
            появилась. Порядок здесь обдуман: СНАЧАЛА называется бесплатный путь (счётчик
            обновится), и только ПОТОМ платный. Экран лимита — самая уязвимая точка
            продукта, и начинать её с продажи значит делать ровно то, за что продукт-источник
            был наказан магазином приложений. */}
        <p className="limit__note">
          Подождать до обновления счётчика — бесплатно и ничего не требует.
        </p>
        <p className="limit__pro">
          Нужно больше прямо сейчас — <a href="/pro">посмотрите Pro</a>: цена и условия на той странице,
          оплата не начинается по этой ссылке.
        </p>
      </div>
      {submitted ? (
