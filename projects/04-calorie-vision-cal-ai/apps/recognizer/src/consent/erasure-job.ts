...
interface PhotoRow {
  ...
  /**
   * НОРМАЛИЗОВАННАЯ копия (`scan-pipeline`, `photo/normalize.ts`) — ВТОРОЙ объект того же
   * снимка в том же бакете. Найдено СЛИЯНИЕМ: эразура писалась, когда нормализации в дереве
   * ветки не было, и удаляла только оригинал; после слияния каждое распознавание создаёт
   * копию, и аккаунт объявлялся `erased`, а изображение тарелки физически ОСТАВАЛОСЬ в
   * бакете. Это ровно тот отказ, против которого написан RV-consent-and-telegram-auth-02,
   * вернувшийся через стык. `PurgeExpiredPhotos` удаляет оба ключа с самого начала —
   * здесь то же правило.
   */
    ...
    for (const row of batch.rows) {
      ...
      try {
        ...
        const pending = await pool.query<PhotoRow>(
          `SELECT p.id, p.object_key, p.normalized_object_key FROM photo p
        ...
        for (const photo of pending.rows) {
          try {
            ...
            // Отметка `purged` ставится ТОЛЬКО после обоих объектов: если второй вызов упал,
            // строка обязана остаться `present` и попасть в выборку следующего прогона.
            if (photo.normalized_object_key !== null) await photoStore.purgeObject(photo.normalized_object_key);
