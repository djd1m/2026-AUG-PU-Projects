...
| `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY`, `S3_SECRET_KEY` | да | да | нет | отказ старта, ЧЕТЫРЕ независимые проверки — каждая переменная предъявлена отдельным утверждением теста и для отсутствия, и для пустой строки (VF-01) |
...
| `N4_RATE_LIMIT_MUTATE_PER_MIN`, `N4_RATE_LIMIT_READ_PER_MIN` | да | нет | нет | отказ старта. Числа канона §7 (DEC-A-013): 30 мутаций и 120 чтений в минуту на `ip_prefix`. Переменными сделаны намеренно — предел, который нельзя подкрутить под машину, однажды отключают целиком (VF-02) |
| `N4_PROXY_RATE_MUTATE_PER_MIN`, `N4_PROXY_RATE_READ_PER_MIN` | нет | нет | нет | только у сервиса `proxy`; те же числа канона, но считаются ОТДЕЛЬНО и по адресу клиента двери. Отсутствие валит ЗАГРУЗКУ КОНФИГУРАЦИИ Caddy, то есть дверь не поднимается вовсе |
