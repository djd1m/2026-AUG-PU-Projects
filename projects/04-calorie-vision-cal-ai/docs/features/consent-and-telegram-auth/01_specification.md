...
consent_version, consent_text_hash }`. `grant` сохраняет `account.consent_version`,
`consent_text_hash`, `consent_at`. Согласие спрашивается ДО ПЕРВОЙ записи в дневник, а не при
установке и не перед первым сканом (ADR-009): камера и результат первого распознавания доступны без
согласия. `consent_version`, не входящая в код-владеемый список известных версий текста, даёт `422`
— согласие на неизвестный текст не является согласием. Реализует FR-AUTH-003, NFR-SEC-002,
...
для владельца без действующего `consent_at` (никогда не дававшего согласие, ЛИБО отозвавшего его —
`withdraw_consent` обнуляет `consent_at`, см. FR-consent-and-telegram-auth-8) отклоняется
`403 consent_required` НА ГРАНИЦЕ, которую держит эта фича — независимо от того, какой код пытается
писать (`scan-pipeline`, консольный скрипт, будущая фича). Недоступность источника истины о согласии
(например, отказ чтения `account`) — тоже отказ, а не пропуск. Реализует NFR-SEC-002, FR-GROWTH-006,
...
### AC-consent-and-telegram-auth-8 — согласие даётся до первой записи дневника
Given пользователь подтвердил первый результат распознавания, дневник ещё не ведётся
...
`200`; `account.consent_version`, `consent_text_hash`, `consent_at` заполнены; последующая попытка
записи дневника проходит.
...
### AC-consent-and-telegram-auth-11 — запись дневника без согласия невозможна на границе фичи
...
Then попытка отклоняется на уровне, который держит эта фича (ограничение или проверка перед записью);
строка `diary_entry` не создаётся.
