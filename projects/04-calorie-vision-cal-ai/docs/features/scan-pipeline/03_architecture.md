...
2. **`ModelProvider.recognize(image, schema)`** расширяется до `recognize(image, schema, { model,
   deadlineMs })`. Без этой правки `FR-scan-pipeline-6`/`-13`/`-15` (явная передача модели, дедлайн
   25 с) не реализуемы поверх интерфейса `foundation` — потребуется либо локальная обёртка ЭТОЙ фичи
   поверх интерфейса (сложнее и дальше от единственного места истины), либо правка `foundation`,
   которую предпочитает эта архитектура.
