...
запись `{ food_item_id, portion_g, source_snapshot, parts? }`, где `parts[]` допускает составное
...
`queued`, захваченный хотя бы раз, для которого `now() - created_at > 30 с` — ПРИБЛИЖЕНИЕ к «30 с от
первого захвата» (в каноне нет колонки `first_leased_at`, и эта фича её не вводит; риск гонки с
легитимным воркером назван и закрыт условием `status = 'queued'` в обоих операторах записи, PC-03).
