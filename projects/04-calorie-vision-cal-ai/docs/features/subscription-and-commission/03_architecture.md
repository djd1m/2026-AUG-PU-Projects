...
Новых сервисов compose НЕТ: подписка и продления живут в `api`. Канон §6 «ровно 6 сервисов»
не меняется.

**ИСПРАВЛЕНО при реализации 2026-09-16.** Первоначальная редакция отдавала продления
`recognizer` — он уже умеет аренду задания с fence, и переиспользовать механику выглядело
разумным. Но продление ЗОВЁТ ПЛАТЁЖНОГО ПРОВАЙДЕРА, а его ключи принадлежат только `api`
(`secrets-management.md`: сервис, которому нечем позвать наружу, не позовёт — и это
проверяется списком `environment:`). Разместить продления в `recognizer` значило бы выдать
распознавателю платёжные секреты ради одной функции, то есть разменять проверяемый инвариант
на удобство размещения. Механика аренды при этом не потеряна: она перенесена как ПРИЁМ
(предикат занятости плюс fence), а не как общий код.
