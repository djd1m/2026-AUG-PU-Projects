...
## Попытка 6 — третий и ФИНАЛЬНЫЙ challenge Codex Astra, DEC-A-020

Прочитан `docs/features/scan-pipeline/plan-challenge-3.md` целиком. Вердикт STOP по остаточной дыре
PC-06 и двум новым находкам (PC3-01, PC3-02); PC-01/PC-04/PC2-01 подтверждены `closed`.

**Ключевая структурная правка — порядок внутри захвата (закрывает PC-06 окончательно, PC-03, PC-05
разом):** захват (`fence++`) → проверка ОБЩЕГО бюджета (`now ≤ created_at + 30с`, иначе
`failed(timeout)` БЕЗ списания) → нормализация ПОД СВОИМ фиксированным дедлайном 3000 мс
(`AbortSignal`, ОТДЕЛЬНЫМ от бюджета вызова модели — истёк → `failed(normalize)`, списания нет,
внешнего вызова не было) → списание квоты `primary` НЕПОСРЕДСТВЕННО ПЕРЕД вызовом с `day = day(now)`
(правило: `fence=1` и тот же день, что `POST`, — не списывает; `fence≥2` — списывает всегда; смена дня
между приёмом/нормализацией и ЭТИМ моментом — списывает всегда, НЕЗАВИСИМО от fence) → вызов с
`deadline=min(25с, remaining)` и `AbortSignal` → эскалация по тому же правилу (`remaining<8с` →
пропуск, из Попытки 5/VS-05). Это закрывает окончательно PC-06: день теперь проверяется В МОМЕНТЕ,
максимально близком к самому вызову, а не на захвате, который мог предшествовать пересекающей полночь
нормализации.

- **PC3-01 (high, новая).** `attempt_id` был один на всё задание — эскалация (второй внешний вызов)
  получала ТОТ ЖЕ `attempt_id`, что и примарный. Заменено: `attempt_id =
  recognition_id:lease_fence:call_no`, `call_no ∈ {1 (primary), 2 (escalation)}` — генерируется на
  КАЖДЫЙ внешний вызов заново. Ровно один `OUTCOME` на `attempt_id`; поздний результат ОБНОВЛЯЕТ
  `OUTCOME` того же `attempt_id` на `late` (правило приоритета: `late` пишется последним по
  построению шага 9, не создаёт второе событие). Правки `RecognizeScanWithinScanPipeline` шаги 1/4/7,
  `FR-scan-pipeline-12`.
- **PC3-02 (medium, новая).** `AC-scan-pipeline-32` требовал одновременно «возраст задания 60 с» (из
  примера 23:59:30→00:00:30) И «в пределах бюджета 30 с» — противоречие с самим же алгоритмом.
  Переписан на пример 23:59:50→00:00:05 (возраст 15 с < 30 с) — оба условия совместимы. Добавлен
  ОТДЕЛЬНЫЙ `AC-scan-pipeline-38`: захват задания старше 30 с → `failed(timeout)` без списания и без
  вызова (это и есть случай, который старый AC-32 ошибочно пытался покрыть тем же самым тестом).
- **PC-02 (partially → closed формулировка).** `S:464` (`AC-scan-pipeline-7`) требовал «строка
  `recognition` переведена в `refused`», что противоречило алгоритму (строка вообще не сохраняется).
  Переписано: «отказ квоты в `POST` НЕ ОСТАВЛЯЕТ строки в БД; объект удаляется best-effort сразу,
  гарантированно — уборкой сирот в течение 1 часа» — дословно по формулировке координатора.
- **PC-07 (partially → closed рассинхрон).** `FR-scan-pipeline-12` (строка 219 старой редакции)
  ПРОТИВОРЕЧИЛА `FR-scan-pipeline-21`: первая говорила «агрегатор — план, эта фича его НЕ реализует»,
  вторая — «эта фича РЕАЛИЗУЕТ». Переписан `FR-scan-pipeline-12` целиком под новую схему `attempt_id`
  с `call_no` и явным подтверждением, что агрегатор ВХОДИТ в фичу; стоимость/владелец/≤5 мин — вне
  недели, названо явно.
- **PC-08 (partially → closed остаточный дубль).** `01_specification.md` раздел «Стык» всё ещё нёс
  сигнатуру ОДНОГО элемента (`match(item): Promise<{...} | null>`) — наследие Попытки 1, ни разу не
  обновлённое при переходе на пакетный вызов в Попытке 3. Исправлено на `match(items[]):
  Promise<Array<{...}>>`; явно названо, что раскрытие `parts[]` — работа `source-and-correct`.
- **PC2-02 (partially → closed сигнатура порта).** `ModelProvider.recognize(image, schema, { model,
  deadlineMs })` расширен ТРЕТЬИМ параметром `signal: AbortSignal` — без него `AbortController` этой
  фичи не может физически прервать вызов адаптера. Правка в `02_pseudocode.md` Data Structures и
  `03_architecture.md` «Зависимости от `foundation`» (по-прежнему ВНЕСЕНО в план `foundation`,
  координатор, не эта квитанция).
- **PC-09** — формулировка «внесено в план `foundation`» сохранена (координатор подтвердил, foundation
  обновляется параллельно), уточнена третьим параметром `signal`.
- **PC-01, PC-04, PC2-01** подтверждены `closed` без изменений.

**Уникальность:** 21 FR, 3 NFR, **38 AC** (`-1`…`-38`, добавлен `-38`) — все заголовки уникальны
(`grep`+`uniq -c`, ноль дублей).

Ворота:
```
TRACE contour=scan-pipeline … COUNT requirements=62 algorithms=62 missing-algorithm=0 orphan-algorithm=0
PASS contour=scan-pipeline bidirectional traceability complete
VERDICT traceability=PASS features=3 gaps=0 inconclusive=0
```
RC=0.

**План заморожен по решению координатора** — дальнейшие замечания рассматриваются на стадии кода
(Phase 3), судьёй Phase 4, а не новым challenge плана.

## Дополнение к Попытке 6 — VS-06/VS-07 (ре-валидатор, validation-report.md v3)

- **VS-06 (medium).** `AC-scan-pipeline-19` всё ещё описывал ОТМЕНЁННЫЙ (Попытка 3, PC2-01) механизм
  ключа объекта по хешу содержимого и говорил про «перезаписывает ТОТ ЖЕ детерминированный ключ» на
  повторе. Переписан под текущий алгоритм: ключ `device_session_id/recognition_id.ext`,
  `recognition_id` генерируется ДО загрузки; крах между `PUT` и транзакцией оставляет орфан-объект,
  убираемый шагом 12 (уборка орфанов, ≤ 1 час) — НЕ best-effort-путём шагов 9.1/9.3, который относится
  к откату СВОЕЙ же транзакции; повтор с тем же `Idempotency-Key` генерирует НОВЫЙ `recognition_id`/
  объект, находит существующую строку по заявке ключа и возвращает ПЕРВЫЙ `scan_id`.
- **VS-07 (low).** Таблица «Размещение по пакетам и сервисам» в `03_architecture.md` заканчивалась на
  `FR-scan-pipeline-13` — `FR-14`…`FR-21` (атомарная публикация, повторное списание, sweeper,
  декодируемость, сутки по моменту попытки, владение объектом, общий бюджет, агрегатор
  `scripts/telemetry/model-calls.sh`) не были размещены ни в одном файле. Добавлены восемь строк.
...
VERDICT traceability=PASS features=3 gaps=0 inconclusive=0
```
RC=0. Правки не меняли ни число, ни идентификаторы FR/AC (38 AC, 21 FR, 3 NFR без изменений) —
только формулировки двух артефактов, синхронизирующие их с уже принятым алгоритмом.
