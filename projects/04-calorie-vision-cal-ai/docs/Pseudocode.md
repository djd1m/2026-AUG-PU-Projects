...
3. Вызвать `CheckAndConsumeQuota(session, reason = primary)`. IF `refused` THEN создать `recognition` со статусом `refused` и `failure_reason = quota_exhausted_user` либо `quota_exhausted_global` по названному `scope`, RETURN `429` с телом `{limit, reset_at, scope}` — поле `scope` называет отказавший потолок, иначе экран лимита не отличает личный потолок от общего и предлагает «приходите завтра» там, где ждать надо не того. Порядок неслучаен: квота ПОСЛЕ валидации (мусор не жжёт чужую квоту) и ДО вызова модели (вызов не делается бесплатно).
...
STEPS:
1. Взять аренду: `SELECT … FROM recognition WHERE status = 'queued' AND (leased_until IS NULL OR leased_until < now()) ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1`, проставить `leased_until = now() + 60 с`. `SKIP LOCKED` закрывает одновременный захват двумя живыми воркерами; предикат по `leased_until` закрывает второй случай — воркер, который взял задание и умер, не удержав блокировку. Продление аренды не выполняется: воркер, не уложившийся в 60 с, считается умершим, и задание честно забирает другой. Цена — редкий повторный вызов модели; она названа в потолках, потому что счёт идёт по попыткам.
...
10. Активация наступает позже: первое УСПЕШНОЕ распознавание переводит `pending → activated` и проставляет `activated_at`. Платной конверсии в неделе нет, поэтому дальше `activated` статус не идёт.
...
|---|---|---|---|---|---|
| 1 | `POST /api/v1/scans` | `Cookie: session` · `Content-Type: multipart/form-data` | файл изображения (JPEG/PNG/HEIC ≤ 12 МБ, ≥ 320×320) | `{ scan_id, status: 'queued' }` / `{ request_id }` | `401` нет сессии · `413`/`422` `invalid_image` с названной причиной · `429` `quota_exhausted` с телом `{ limit, reset_at, scope: 'user' \| 'global' }` — `scope` называет отказавший потолок · `503` `provider_unavailable` |
| 2 | `GET /api/v1/scans/{id}` | `Cookie: session` | — | `{ status, items[], kcal_total, macros, sources[], confidence, low_confidence, escalated, conflict_flag, model_kcal_estimate, share_card_id? }` / `{ request_id, updated_at }` | `401` · `403` чужой скан · `404` неизвестный `id` |
