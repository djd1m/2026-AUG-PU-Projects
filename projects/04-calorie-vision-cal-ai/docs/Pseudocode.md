...
| 2 | `GET /api/v1/scans/{id}` | `Cookie: session` | — | `{ status, items[], kcal_total, macros, sources[], confidence, low_confidence, escalated, conflict_flag, model_estimate_kcal, share_card_id? }` / `{ request_id, updated_at }` | `401` · `403` чужой скан · `404` неизвестный `id` |
...
| 11 | `PATCH /api/v1/diary/{entry_id}` | `Cookie: session` · `Content-Type: application/json` | `{ op: 'confirm' \| 'set_portion' \| 'delete', index?, mass_g? }` | пересчитанная запись и итоги дня `{ entry, totals: { kcal, protein, fat, carb } }` / `{ request_id }` | `401` · `403` чужая запись · `404` неизвестный `entry_id` — ОДИН и тот же ответ, чтобы существование чужой записи не раскрывалось · `422` `portion_out_of_range` с диапазоном 5–2000 г · `409` запись уже удалена |
