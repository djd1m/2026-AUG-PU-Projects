...
8а. IF НИ ОДНА позиция не сопоставлена с `food_item` THEN записать `failed(no_food_matched)` с подсказкой «блюда нет в базе, уточните ингредиент вручную» и RETURN. Статус `done` при нуле ссылок на `food_item` НЕВОЗМОЖЕН — это инвариант ADR-001, а не оптимизация: результат `done` обещает число из базы, а здесь его неоткуда взять, и `kcal_total = 0` прочиталось бы как «в тарелке ноль калорий». Одна сопоставленная позиция из трёх — законный `done` с двумя пометками «нет в базе» (SC-US-002-2); ноль сопоставленных — отказ.
...
### Algorithm: CameraFirstScreenBudget

REQUIREMENT: `NFR-PERF-002`
REALISES: —
INPUT: загрузка PWA / Mini App на мобильном устройстве
OUTPUT: видоискатель доступен ≤ 1 с (p95) после открытия
STEPS:
1. Первый экран рендерится статически (SSR/пререндер), без ожидания API.
2. Разрешение камеры запрашивается сразу при монтировании; поток подключается к `<video>` без промежуточных экранов.
3. Всё, что не нужно для съёмки (дневник, кабинет партнёра, шрифты вторичных экранов), грузится отложенно после первого кадра.
4. Измерение: `performance.now()` от навигации до события `loadedmetadata` видеопотока пишется в журнал клиента (наш журнал) — источник для NFR-PERF-002.
5. IF бюджет превышен THEN показывается видоискатель с индикатором «камера подключается», без блокирующего спиннера на весь экран.
COMPLEXITY: O(1)
