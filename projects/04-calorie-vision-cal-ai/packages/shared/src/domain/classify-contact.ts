...
// числовой telegram_id — ЛЮБОЕ положительное целое (FR-pro-interest-and-limits-ui-6 не
// называет нижней границы длины; верхняя граница — только общая `MAX_CONTACT_LENGTH` ниже).
//
// RV-pro-interest-and-limits-ui-02 (правка после ревью): прежний `{4,15}` вводил
// незаявленный минимум 5 цифр — `"1"` и `"1234"` отклонялись, хотя оба являются валидными
// положительными целыми по тексту FR-6. Псевдокод (`02_pseudocode.md`) содержит более узкий
// шаблон, но псевдокод не отменяет требование спецификации — исправлен именно классификатор.
const EMAIL_PATTERN = /^[^\s@]{1,64}@[^\s@]{1,190}\.[^\s@]{2,24}$/;
const TELEGRAM_PATTERN = /^@?[A-Za-z0-9_]{5,32}$/;
const TELEGRAM_ID_PATTERN = /^[1-9][0-9]*$/;
