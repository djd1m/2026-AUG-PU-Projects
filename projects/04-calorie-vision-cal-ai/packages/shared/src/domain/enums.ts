...
export const RECOGNITION_FAILURE_REASONS = [
  ...
  // Расширение фичи `scan-pipeline` (DEC-A-017, координатор): два значения за пределы
  // исходных восьми канона. `timeout` — общий бюджет задачи 30 с истёк (FR-scan-pipeline-20);
  // `normalize` — нормализация фото не уложилась в свой дедлайн 3000 мс (FR-scan-pipeline-5).
  // Зеркалируется миграцией `002_scan_pipeline_failure_reasons.sql`.
  'timeout',
  'normalize',
] as const;
