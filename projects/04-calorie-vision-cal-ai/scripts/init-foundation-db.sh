...
# ИМЯ ЗДЕСЬ — `POSTGRES_PASSWORD`, и это не вкусовщина. Административный пароль приезжает в
# контейнер именно под этим именем (`docker-compose.yml`: `POSTGRES_PASSWORD: ${N4_DB_ADMIN_PASSWORD:?…}`),
# а переменной `N4_DB_ADMIN_PASSWORD` внутри контейнера НЕ СУЩЕСТВУЕТ.
#
# Прежняя редакция сравнивала с `${N4_DB_ADMIN_PASSWORD:-}`, то есть с ПУСТОЙ СТРОКОЙ, и
# потому не могла отказать НИКОГДА: пустая строка не равна непустому паролю приложения.
# Это ровно `guard-must-be-able-to-fail` — страж, ни разу не показавший красное, стражем не
# является. Цена промаха: приложение получает секрет, годный для входа под `n4_admin`, и
# запрет DDL у роли `n4_app` перестаёт что-либо изолировать (слепое ревью, RV-foundation-01).
#
# Отсутствие административного пароля — тоже отказ, а не «сравнивать не с чем».
: "${POSTGRES_PASSWORD:?POSTGRES_PASSWORD обязателен: без него сравнивать пароль приложения не с чем, а «не с чем сравнить» не означает «различаются»}"
...
if [[ "$POSTGRES_PASSWORD" == "$N4_DB_APP_PASSWORD" ]]; then
  echo 'пароль администратора и пароль роли приложения обязаны различаться: одинаковые значения дают приложению вход под n4_admin' >&2
  exit 1
fi
