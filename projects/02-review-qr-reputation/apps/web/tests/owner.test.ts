...
describe('облик: одна система на все владельческие поверхности', () => {
  function strip(src: string): string {
    return src.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^\s*\/\/.*$/gm, ' ');
  }

  it('оболочка страницы РОВНО одна — ни одна страница не строит свой документ', async () => {
    // ЧЕМ ЗАСЛУЖЕНО. Три страницы кабинета носили собственный <style>, и системы в них уже
    // разошлись: у одной кнопка-пилюля, у другой прямоугольник, рамки разного цвета. Владелец
    // это увидел раньше нас. Две копии одной системы расходятся молча — значит копия одна, и
    // это свойство исходника, а не договорённость.
    const { readFileSync } = await import('node:fs');
    const src = strip(readFileSync(new URL('../src/pages.js', import.meta.url).pathname
      .replace('/pages.js', '/pages.ts'), 'utf8'));

    const docs = (src.match(/<!doctype html>/gi) ?? []).length;
    expect(docs, `страниц со своим документом: ${docs} — оболочка обязана быть одна`).toBe(1);
    const styles = (src.match(/<style>/g) ?? []).length;
    expect(styles, 'вторая таблица стилей — вторая система, которая разойдётся с первой').toBe(1);
  });

  it('публичная страница не нарушает границу площадок', async () => {
    // Тот же запрет, что стережётся на макетах QR (NFR-LEGAL-001), но здесь поверхность
    // ПУБЛИЧНАЯ: подсказка содержания или обещание вознаграждения на витрине продукта — это
    // то, за что площадки считают отзывы накруткой, и увидит это кто угодно, не только гость.
    const { landingPage } = await import('../src/pages.js');
    const html = landingPage(990);
    expect(html).not.toMatch(/напишите про|поставьте|5 звёзд|скидк[ауи]|бонус|подар|вознагражден\w* за/i);
    expect(html, 'цена обязана приходить параметром, а не быть вписанной').toContain('990');
  });

  it('числа на витрине не выдуманы: у каждого названо, откуда оно', async () => {
    // У образца на этом месте чужие результаты («128% больше отзывов»). Скопировать форму
    // вместе с числами — выдать чужое измерение за своё; PRD §2.4 требует число И источник.
    const { landingPage } = await import('../src/pages.js');
    // Разметка снимается ДО проверки. Первая редакция искала «128% больше» как соседние
    // слова — а между числом и подписью стоят теги, и мутация, вписавшая ровно это чужое
    // обещание, прошла МИМО стража. Проверять надо текст, который читает человек.
    const text = landingPage(990).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ');
    expect(text, 'замер обязан называть, где и когда сделан').toMatch(/Замер на стенде \d{2}\.\d{2}\.\d{4}/);
    expect(text, 'процентных обещаний роста на витрине быть не должно')
      .not.toMatch(/\d{2,3}\s*%\s*(больше|рост|увеличен|выше)/i);
    // И зеркально: каждое число полосы обязано иметь пояснение рядом. Полоса из трёх чисел,
    // у которой пояснений меньше трёх, — это форма образца без его содержания.
    expect((text.match(/Замер на стенде|Сменили ссылку|Бесплатный план/g) ?? []).length,
      'у числа в полосе нет собственного пояснения').toBe(3);
  });
});
