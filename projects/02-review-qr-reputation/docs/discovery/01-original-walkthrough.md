...
### 4.1. NiceJob — палитра `[СНЯТО С САЙТА]`
...
### 4.2. NiceJob — типографика `[СНЯТО С САЙТА]`
...
### 4.3. NiceJob — геометрия, кнопки, контейнер `[СНЯТО С САЙТА]`
...
### 4.4. Birdeye и Podium — для сравнения `[СНЯТО С САЙТА]`
...
### 5.1. QR: продуктовой функции нет ни у одного из трёх `[СНЯТО С САЙТА]`
...
`[ВЫВОД]` **QR-код у всех трёх лидеров рынка — тема контент-маркетинга, а не функция продукта.**
Их механика доставки — SMS и e-mail по базе контактов из CRM. Это прямо подтверждается шагом 3
онбординга NiceJob («1 SMS + 3 письма») и цитатой клиента Podium: «we can now easily and quickly
send the patient a link at the end of their visit».

Для нас это одновременно возможность и предупреждение: ниша свободна, но она свободна у компаний,
которые обслуживают 50 000+ бизнесов — то есть они её видели и не взяли. Наиболее вероятная
причина видна из их же статьи (5.2): QR даёт меньше контроля над таймингом и хуже переживает
антиспам Google.
