...
| ~~K-11~~ | ~~требование о видимости грубой ступени невыполнимо без гранта~~ | **ЗАКРЫТО, пройдя выдан → отозван → выдан снова.** Важнее гранта оказался вывод `spec`: **все три редакции требования предписывали МЕХАНИЗМ, и каждый раз механизм ломал соседнее требование.** Правильная форма — свойство («отказ виден», «тракт отказа не ходит в БД»), а не «пиши событие туда-то». Гранты: `analytics_events` INSERT без SELECT; `rate_limit_events` INSERT **и** SELECT (страж T3c) | — |
