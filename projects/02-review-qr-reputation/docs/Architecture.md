...
> сохранена: **роута, принимающего оценку**, не существует, а `/go` принимает только дверь, которую
...
export function resolvePlatformUrl(slug: string, platform: Platform): Url   // то же для /go
...
`Location` у `/go`. Страж T9 (по исходнику, паттерн
...
| ветвление в шаблоне по `?rating=` · **ветвление по `?rating` в `/go`** · `hidden` на блоке приватной двери · виджет звёзд · тень на приватной строке · импорт `headers()` в `render.ts` **или в `resolve.ts`** | T4 · **T4b** · T5 · T6 · T5 · T9 |
