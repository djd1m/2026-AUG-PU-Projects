...
Дедупликация устройств выполняется **при агрегации** (роль `app_owner`), а не при вставке.
...
`POST /r/:slug/decide`; любой путь, принимающий оценку **до выбора двери**; любой параметр, влияющий на ответ.
...
**Индексы, несущие смысл:** `places(slug)` unique — вход рендера; `platform_links(place_id)` — набор
дверей одним запросом; `guest_events(place_id, created_at)` — метрика недели;
`private_feedback(place_id, created_at DESC)` — лента кабинета; `notifications(status, created_at)` —
очередь воркера. **Индекса `guest_events(place_id, device_hash)` НЕТ намеренно** (§3.1).

**Уникальность устройства без сквозного идентификатора.** `device_hash =
HMAC-SHA256(секрет ‖ ISO-номер-недели, place_id ‖ IP ‖ User-Agent)`, усечённый до 16 байт. Две
границы области действия, обе обязательные: соль привязана к **неделе**, `place_id` — к **точке**,
поэтому один телефон даёт разные хэши в разных заведениях. Без `place_id` это сквозной
идентификатор между точками, прямо запрещённый FR-013; связать устройство ни между неделями, ни
между заведениями нельзя даже нам, сырые IP и UA не сохраняются нигде.
...
2. Валидация: `body` **10–2000 символов** (FR-006), `rating` — целое 1–5 либо отсутствует; любое
...
`channel_bindings`, обновляет статус. Ретраи с экспоненциальной задержкой, предел —
6 попыток, после — `failed` и запись в `audit_log`. Владелец отвечает гостю прямо из мессенджера,
если тот оставил контакт.
