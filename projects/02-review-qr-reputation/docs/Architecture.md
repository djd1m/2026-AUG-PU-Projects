...
Инвентарь существует не ради полноты описания: это **единственное место конвейера, которое
смотрит наружу документов**. Требование, опирающееся на возможность, которой у мира нет,
проходит проверки на полноту, измеримость и согласованность — и падает только здесь.

Вердикт `CONFIRMED` требует трёх вещей вместе: ссылка, дословная цитата и дата проверки. Ссылка
без цитаты — самая дешёвая подделка: её никто не открывает.

| Capability · Возможность | Провайдер | Доказательство | Вердикт | Требования, которые на ней держатся |
|---|---|---|---|---|
| Приём оплаты по карте с редиректом и уведомлением о результате | ЮKassa API v3 | https://yookassa.ru/developers/using-api/webhooks — «Когда получите уведомление, проверьте его подлинность, например по статусу объекта или по IP-адресу.» Проверено 2026-09-02; в тот же день проведён живой тестовый платёж, уведомление принято и тариф применён | CONFIRMED | FR-010, FR-011 (оплата, снятие бренд-строки) |
| Доставка сообщения владельцу в мессенджер, привязка чата по одноразовому токену | Telegram Bot API 10.3 | https://core.telegram.org/bots/api — про `getUpdates`: «Identifier of the first update to be returned. Must be greater by one than the highest among the identifiers of previously received updates.» Проверено 2026-09-02 (версия 10.3 от 24.08.2026) | CONFIRMED | FR-008, FR-009 (уведомление о приватном обращении, привязка канала) |

### Способность, которой у мира НЕТ: чтение опубликованных отзывов

В таблице выше её строки нет намеренно — инвентарь перечисляет то, на чём продукт **держится**,
а здесь ровно наоборот. Знание при этом несущее и потому записано тут же:

**Ни у Яндекс.Карт, ни у 2ГИС нет публичного API отзывов организации.** Разбор доступных
интерфейсов — [`discovery/01b-review-mechanics.md`](discovery/01b-review-mechanics.md),
2026-08-30. Продукт физически не может узнать, опубликовал ли гость отзыв.

Площадки названы порознь: это независимые поставщики, и появление интерфейса у одной ничего не
сообщает о другой.

Продукт **физически не может узнать**, опубликовал ли гость отзыв на площадке. Это не пробел
реализации и не задача на потом: у обеих площадок нет интерфейса, через который такой факт
доступен.

Из этого следуют три решения, уже принятые и записанные в других местах — этот раздел их лишь
связывает воедино:

1. **Метрика недели меряет сканы, а не опубликованные отзывы** (PRD, Success Metrics). Метрика
   «доля дошедших до публикации» была бы обещанием, которое нечем проверить.
2. **`public_review_counts.source` — перечисление из одного значения `manual`** (миграция 004).
   Автоматический источник появится только вместе с осознанной правкой схемы, а не «когда
   появится API».
3. **Успех продукта заявлен как рост потока к публичной двери**, а не как число отзывов. Разница
   не косметическая: второе мы измерить не в состоянии.

### Граница знания

«Интерфейса нет» здесь означает «не найдено при разборе от 2026-08-30», а не математическое
доказательство отсутствия. Отрицание такого рода всегда ограничено датой и глубиной поиска — и
при появлении официального API этот раздел переписывается, а не тихо забывается.
