...
- **Вне scope недели:** ответы на публичные отзывы, парсинг рейтингов площадок, white-label
  агентств, интеграции с кассами и CRM. Ни один компонент ниже не проектируется «с запасом» под них.
...
├── packages/
│   ├── db/             # Миграции, роли, ГРАНТЫ, RLS-политики
│   ├── shared-types/   # TS-типы, общие для сервисов
│   └── ui/             # Компоненты кабинета. НЕ используется в apps/guest
...
> ⚠️ **Сознательное отклонение от [`03`](discovery/03-legal-gating-boundary.md) §7.3, требующее
> подтверждения на Phase 2.** Discovery объявляет исчерпывающим список из **двух** путей и запрещает
> «роут, возвращающий 3xx». Но [`DECISIONS-PHASE-0`](DECISIONS-PHASE-0.md) D-01 требует
> инструментировать **долю гостей, выбравших публичную дверь**, с первого релиза, а статический
> `<a href>` наружу такого числа не даёт ни при каких условиях. Конфликт разрешён в пользу
> измеримости (цена и отвергнутые альтернативы — ADR-004); несущая часть §7.3 сохранена: **роута,
> принимающего оценку**, не существует, а `/go` принимает только дверь, которую гость **уже** выбрал.
...
параметр `GET /r/:slug`, влияющий на разметку.
...
```ts
// apps/guest/src/render.ts — единственный модуль, порождающий разметку
export function renderChoicePage(slug: string): Html            // нет Request, нет заголовков, нет Date
...
**У функции рендера нет аргумента, в который можно подать контекст запроса; у функции журналирования
нет возвращаемого значения, через которое контекст мог бы вернуться.** Пути от контекста к ответу не
существует — не потому, что он запрещён, а потому, что оба его конца отсутствуют. Страж T9 (по
исходнику, паттерн [`source-parsing-invariant-guard`](../../../.claude/patterns/source-parsing-invariant-guard.md))
утверждает: `render.ts` не импортирует `headers()`, `cookies()`, `Date`, гео- и UA-разбор, и
сигнатура `recordGuestEvent` возвращает `void`.
...
2. **Гостевые роли RLS не используют вовсе** — им нечего задавать в контекст, и изоляция там
   сильнее: `app_render` и `app_intake` резолвят `slug → place_id` и **не принимают идентификатор
   арендатора ни в каком виде**.

Тест-контракт: «арендатор A не читает точку арендатора B» гоняется и через кабинет (RLS и
`SET LOCAL`), и через гостевые пути (резолв по `slug`).
...
`302` на `platform_links.url`. Оба аргумента — из пути; query, cookie и заголовки игнорируются
целиком. `recordGuestEvent(req, 'door_click')` перед редиректом. Неизвестная пара — `404`, не
редирект «куда-нибудь» ([`fail-closed-defaults`](../../../.claude/rules/fail-closed-defaults.md)).
Тот же инвариант чистоты, что и у §5.1, проверяется тем же способом (T4b).
...
Тест — **конкурентный**: 20 одновременных запросов по одному ключу, и число занятых соединений
пула не растёт с числом ожидающих.
...
Публикацию отзыва на площадке продукт не наблюдает: API отзывов у Яндекса нет, 2ГИС его не отдаст
(бриф §М3.4), а данные Яндекс API ППО хранить запрещено лицензией. Поэтому «доля дошедших до
публикации» — **ручной замер**: оператор раз в неделю смотрит карточку и вносит число в
`public_review_counts` с `source='manual'`; enum с единственным значением — именно поэтому.
Статус показывается честно: модерация от 2 часов до 7 дней, «отправлено» ≠ «опубликовано».
...
ЮKassa (ADR-009). Апгрейд: `checkout_sessions` → платёжная страница → вебхук. Подлинность — две
независимые проверки: сеть источника (**allowlist подсетей зашит в код**, не в переменную: пустая
переменная читалась бы как «принимать отовсюду») и **перезапрос статуса платежа через API**; HMAC
нет, провайдер не подписывает уведомления. Порядок: **подлинность → заявка на `event_id` →
применение тарифа**; недоступность провайдера — **исключение**, откатывающее заявку, а не
возвращаемое значение ([`security-operation-order`](../../../.claude/rules/security-operation-order.md)).
...
| **T9** | `check-render-purity.sh` | По исходнику: `render.ts` не импортирует `headers()`/`cookies()`/`Date`/гео/UA-разбор; сигнатура `recordGuestEvent` возвращает `void` |
...
| ветвление в шаблоне по `?rating=` · `hidden` на блоке приватной двери · виджет звёзд · тень на приватной строке · импорт `headers()` в `render.ts` | T4 · T5 · T6 · T5 · T9 |
...
| Приватные обращения | `private_feedback` | `messages`, `complaints`, `negative_feedback` | «negative» вернуло бы тональность в схему; `complaints` сужает — гость пишет и благодарности |
| Гостевой журнал | `guest_events`, `kind enum(scan,door_click)` | `scans` + `clicks` двумя таблицами | Одна таблица = один грант `INSERT` и одно место, где проверяется отсутствие `SELECT` |
...
| Роут выбора | `GET /r/:slug` | `/review/:slug`, `/q/:slug` | Короткий путь печатается в QR и произносится вслух при продаже |
| Роут перехода | `GET /go/:slug/:platform` | `/redirect`, `/out`, `/click` | `platform` в пути делает видимым, что аргумент — уже сделанный выбор, а не оценка |
...
- `[GAP: Q1 — открывает ли какая-либо ссылка форму отзыва Яндекс.Карт напрямую; Q2 — стабилен ли
  URL в QR из «Промоматериалов» Яндекс Бизнеса. Обе — блокирующая ручная проверка на физическом
  телефоне (iOS + Android + веб). Схема готова: меняется `link_kind` с `card` на `review_form` и
  текст шага 2 онбординга, без миграции]`
- `[GAP: маркетинговое название и домен не выбраны — влияет на `BASE_URL`, печатный макет и
  бренд-строку, не на модель данных. `reviewqr` — имя монорепо, наружу не выносится]`
- `[GAP: отклонение от §7.3 discovery (третий публичный путь `/go`, ADR-004) требует явного
  подтверждения на Phase 2 — прочитать до трёх вопросов §8 «что проверяется по документам»]`
