...
сброс — ОДНОЙ агрегатной записью `rate_limit_coarse_window`. **Выполняет сброс не `app_intake`.**
Допустимы ровно две формы: лог процесса либо запись ролью, **у которой право уже есть** —
`app_notify`. Третья форма — выдать `INSERT` на `analytics_events` роли приёма ради наблюдаемости —
недопустима: `app_intake` обслуживает единственный неаутентифицированный путь записи, и лишнее
право на нём расширяет радиус поражения дефекта там, где дефект дешевле всего достаётся
(`Architecture-DATA.md` §6, «право без повода»).
