...
-- analytics_events роли приёма НЕ выдаётся и не будет — см. ниже, «право без повода».
-- Формулировка нарочно не содержит строки поиска: комментарий не должен давать ложное срабатывание.
...
**Право без повода: почему `analytics_events` НЕ выдан `app_intake`.** Грант был выдан под требование
«срабатывания грубой ступени видны в метриках» и **отозван вместе с ним**: требование подразумевало
запись на каждый отказ, а это превращает защиту в усилитель атаки — запрос атакующему бесплатен, нам
стоит записи в хранилище, общем с гостевой страницей и оплатой. Правильная форма — счётчик в памяти
и периодический сброс **ролью, у которой право уже есть** (`app_notify`), либо в лог процесса.
Запись оставлена здесь намеренно: `app_intake` обслуживает единственный неаутентифицированный путь
записи, и лишнее право на нём расширяет радиус поражения дефекта там, где дефект дешевле всего
достаётся. Без этой записи следующий читатель однажды выдаст грант заново — повод забудется, а
удобство останется.
