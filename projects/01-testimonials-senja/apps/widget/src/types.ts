...
export interface WidgetTestimonial {
  ...
  /**
   * Отзыв, перенесённый владельцем с внешней площадки. Подпись приходит ГОТОВОЙ строкой:
   * решать, как назвать источник, сервер виджету не доверяет — по той же причине, по которой
   * не доверяет решение о бренд-строке (ADR-002).
   */
  source_label?: string | null;
  /** Ссылка на публичный отзыв у площадки. Абсолютная и чужая — уходит на другой домен. */
  source_url?: string | null;
  /** Снимок отзыва. Тот же относительный путь нашего роута, что и photo_url. */
