...
export interface WidgetTestimonial {
  ...
  /**
   * Фото автора. Приходит ОТНОСИТЕЛЬНЫМ путём нашего роута ("/api/photo/..."), потому
   * что сервер не знает, на каком домене окажется виджет. Разрешать его нужно
   * относительно apiBase, а НЕ относительно страницы хоста — иначе браузер пойдёт
   * за картинкой на сайт владельца, где её нет.
   */
