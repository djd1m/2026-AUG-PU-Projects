...
/**
 * Приём видео-отзывов — ЕДИНСТВЕННЫЙ путь продукта, который тратит деньги владельца:
 * каждая расшифровка это платный вызов внешней модели, и запускает его ПОСТОРОННИЙ —
 * анонимный посетитель формы, а не мы.
 *
 * Поэтому выключатель fail-closed: незаданная переменная означает ВЫКЛЮЧЕНО, а не
 * включено. Правило honest-configuration (CFG-S1): отсутствующее значение отказывает и
 * называет внешнее последствие. Последствие здесь — счёт, и он необратим: никакая правка
 * кода уже потраченных денег не вернёт.
 *
 * Включается осознанно: `VIDEO_INTAKE_ENABLED=true`. Любое другое значение, включая
 * пустое, отсутствующее, `1`, `yes` и `TRUE` с опечаткой, читается как выключено —
 * список разрешённых написаний закрыт намеренно, чтобы «почти правильное» значение не
 * открывало платный путь.
 */
export function videoIntakeEnabled(): boolean {
  return process.env.VIDEO_INTAKE_ENABLED === 'true';
}
