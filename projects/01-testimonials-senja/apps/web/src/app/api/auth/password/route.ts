...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    // 55P03 lock_not_available — «повтори», а не «сломалось». НО этот же код возвращает
    // Postgres, когда строку держит миграция или DDL: тогда 409 «повторите» скрыл бы
    // настоящую причину, а повтор не помог бы. Отличаем по факту: 409 отдаём только если
    // строка аккаунта существует и доступна на чтение — если недоступна и она, дело не в
    // конкурентной смене, и ошибку надо показать.
    //
