...
describe('соотношение пределов — считается В БАЙТАХ, на языке продукта', () => {
  it(`${MAX_IMPORT_ROWS} строк предельной длины КИРИЛЛИЦЕЙ помещаются в MAX_IMPORT_BODY`, () => {
    // Прежняя редакция считала в СИМВОЛАХ и сравнивала с пределом в БАЙТАХ. Для кириллицы —
    // языка продукта — расхождение ровно вдвое: тест видел 1 105 000 и зеленел, а фактически
    // строки занимают 2 205 000 байт и в 2 МиБ не помещаются, не хватает 107 848.
    //
    // То есть тест утверждал «предел строк достижим», а свойство было ЛОЖНЫМ: до проверки
    // числа строк дело не дошло бы никогда — сработал бы 413, и сообщение было бы о размере,
    // а не о числе строк. Владелец не понял бы, что делать.
    //
    // Модель строки берёт ROLE_MAX, а не NAME_MAX: предел роли теперь существует (ревью B-1).
    const row = `${'я'.repeat(NAME_MAX)},${'я'.repeat(TEXT_MAX)},${'я'.repeat(ROLE_MAX)}\n`;
    const perRow = Buffer.byteLength(row, 'utf8');
    const needed = perRow * MAX_IMPORT_ROWS;

    expect(MAX_IMPORT_BODY,
      `строка ${perRow} байт × ${MAX_IMPORT_ROWS} = ${needed}, предел ${MAX_IMPORT_BODY}`)
