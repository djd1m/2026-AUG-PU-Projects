...
Сгенерировано из истории git по каталогу `projects/01-testimonials-senja`.
Дата: 2026-08-27.

## Состояние

Все 13 функций MVP реализованы. 408 проверок в 35 файлах, все проходят.

| Функция | Что вошло |
|---|---|
| `FR-001` | Регистрация владельца и создание проекта (слаг, 3 ссылки: форма/стена/виджет) |
| `FR-002` | Форма сбора — текстовый отзыв (rate limit 5/час/IP, приём без санитизации) |
| `FR-003` | Форма сбора — видео-отзыв + очередь транскрипции через OpenAI STT (ADR-005, граница FTC) |
| `FR-004` | Модерация: pending/approved/rejected/hidden, audit log, cross-project 403 |
| `FR-005` | Публичная страница Wall of Love (SSR, schema.org/Review, экранирование при рендере) |
| `FR-GROWTH-005` | Порог содержательности и noindex для /w/<slug> — anti scaled-content-abuse (ADR-004) |
| `FR-006` | Встраиваемый виджет: Shadow DOM, ≤30KB gzip, badge_required с сервера (ADR-001) |
| `FR-GROWTH-001` | widget_installed + invite_shown (момент ценности) и share-CTA на каждый новый домен |
| `FR-007` | Тарифы free/paid как атрибут проекта, серверная проверка (не в клиенте) |
| `FR-GROWTH-003` | Badge loop: обязательный badge на free, anti-tamper MutationObserver (ADR-002) |
| `FR-008` | Checkout + webhook оплаты, идемпотентность по event_id (ADR-006), апгрейд тарифа |
| `FR-GROWTH-002` | Партнёрская атрибуция: приоритет промокода над cookie (ADR-003), self-referral guard |
| `FR-GROWTH-004` | Персональные партнёрские коды + anti-fraud по IP на регистрациях |
