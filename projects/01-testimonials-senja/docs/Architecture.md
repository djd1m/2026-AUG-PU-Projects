...
`checkout_sessions (project_id, provider_session_id, status='pending')`, обращается к **ЮKassa**
(`decisions/D-009`) — контракт `project_id → {provider_session_id, redirect_url}` по форме остаётся
провайдер-агностичным, но способ подтверждения подлинности провайдер-специфичен: ЮKassa уведомления
НЕ подписывает, см. §3.5.
