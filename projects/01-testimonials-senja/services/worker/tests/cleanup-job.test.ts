...
describe.skipIf(!hasTestDb)("cleanupRateLimitEvents — граница 24 часов", () => {
  ...
  async function insertAt(scope: string, key: string, minutesAgo: number): Promise<void> {
    await pool.query(
      `INSERT INTO rate_limit_events (scope, key, created_at) VALUES ($1, $2, now() - make_interval(mins => $3))`,
      [scope, key, minutesAgo],
    );
  }
  ...
  it("удаляет строки старше 24ч, оставляет свежие (form_submission/signup_via_partner_code/project_created — общая таблица, security.md §4)", async () => {
    await insertAt("form_submission", "1.2.3.4:proj-1", 25 * 60); // 25ч назад — удалить
    await insertAt("signup_via_partner_code", "5.6.7.8", 30 * 60); // 30ч назад — удалить
    await insertAt("project_created", "acct-1", 23 * 60); // 23ч назад — оставить
    await insertAt("form_submission", "9.9.9.9:proj-2", 6); // 6 минут назад — оставить
  ...
  it("ничего не удаляет, если все события свежие", async () => {
    await insertAt("form_submission", "1.1.1.1:proj-1", 60);
    const { deletedRows } = await cleanupRateLimitEvents(pool, 24);
