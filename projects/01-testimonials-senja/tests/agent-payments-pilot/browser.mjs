...
export async function browser(state, save, runtime, evidence) {
  let driver;
  let base = state.browserBase;
  if (!base) {
  const port = await freePort();
  const profileRoot = process.env.AGENT_E2E_FIREFOX_PROFILE_ROOT || '/root/snap/firefox/common';
  const log = openSync(join(runtime, 'firefox.log'), 'a', 0o600);
  driver = spawn('geckodriver', ['--host', '127.0.0.1', '--port', String(port), '--profile-root', profileRoot], { detached: true, stdio: ['ignore', log, log] }); driver.unref();
  const endpoint = `http://127.0.0.1:${port}`;
  let ready = false;
  for (let n = 0; n < 450; n++) {
    try { if ((await fetch(endpoint + '/status')).ok) { ready = true; break; } } catch {}
    if (driver.exitCode !== null) throw Error('Private geckodriver exited; private log retained');
  ...
  const response = await fetch(endpoint + '/session', { method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ capabilities: { alwaysMatch: { browserName: 'firefox', acceptInsecureCerts: false,
  ...
  base = `${endpoint}/session/${session.value.sessionId}`; state.browserBase = base; state.driverPid = driver.pid; await save();
  }
  ...
  async function until(script, timeout = 30000) {
    ...
    throw Error(`Browser condition timeout: ${script}`);
  }
  ...
  async function shot(name) { assert.equal(await js('return location.origin'), state.webOrigin, 'Screenshots allowed only on pilot origin'); await js('document.querySelectorAll("textarea,input[type=password]").forEach(e=>e.style.visibility="hidden")'); await mkdir(evidence, { recursive: true }); await writeFile(join(evidence, name + '.png'), Buffer.from(await wd('/screenshot'), 'base64')); }
  ...
  return { wd, js, until, click, button, fill, label, checkbox, open, shot,
    async close() { try { await wd('', undefined, 'DELETE'); } finally { try { process.kill(state.driverPid, 'SIGTERM'); } catch {} delete state.browserBase; delete state.driverPid; await save(); } },
  };
}
