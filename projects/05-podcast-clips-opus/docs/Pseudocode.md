...
OUTPUT: `granted`, либо `refused(scope)`, где `scope` — ОДИН из ПЯТИ ключей канона, назвавший отказ. Вызов поставщика после `refused` не выполняется НИКОГДА.
STEPS:
...
5. Отказ несёт `scope` и время возврата (ближайшая полночь `Europe/Moscow`). Пять ключей дают ПЯТЬ разных текстов: «загрузки на сегодня исчерпаны», «минуты на сегодня исчерпаны», «обработок на сегодня исчерпано», «общий суточный объём исчерпан», «общий суточный лимит обработок исчерпан». Пользователь глобального потолка не виноват и должен это видеть.
...
4. Отказы по потолкам записываются ОТДЕЛЬНО, с названным `scope`: доля отказов по каждому из ПЯТИ ключей — метрика недели, и общий счётчик «лимит исчерпан» её не даёт.
...
`429`, тело называет `scope` из ПЯТИ ключей канона и время возврата), `conflict` (`409`),
