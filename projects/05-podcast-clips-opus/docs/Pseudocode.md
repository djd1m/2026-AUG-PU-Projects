...
`209238f416daf49d3a41498df51f848e79ab833895b84a44f494fe89e8597c9c` (заморожена на время фазы).
...
| `video` | `account_id: UUID`, `idempotency_key: string` (значение заголовка `Idempotency-Key`, уникально в паре с `account_id`), `source: upload / url` — ЗАКРЫТЫЙ набор из двух, `source_url: string?` (только при `url`), `object_key: ObjectKey?`, `upload_id: string?` (идентификатор multipart-загрузки), `declared_bytes: Bytes` (заявлено клиентом — значение, которое НЕ проверено), `actual_bytes: Bytes?` (получено `HEAD` после завершения), `duration_seconds: Seconds?` (измерено `ffprobe`), `minutes_charged: Minutes?`, `status: uploading / queued / transcribing / selecting / rendering / done / failed` — ЗАКРЫТЫЙ набор из семи, `failure_reason: too_large / not_media / no_audio / too_short / too_long / stt_failed / no_timestamps / no_fragments / schema_violation / refused_user_minutes / refused_global_minutes / refused_global_llm / stalled / render_failed?` — ЗАКРЫТЫЙ набор, `stage_progress: int?` (0–100 внутри текущей стадии), `clips_total: int?`, `clips_done: int?`, `updated_at: Timestamp` (двигается КАЖДЫМ шагом любой стадии — на нём стоит различение молчания), `finished_at: Timestamp?`, `deleted_at: Timestamp?` |
...
| `quota_counter` | `scope: user_minutes / user_uploads / global_minutes / global_llm` — ЗАКРЫТЫЙ набор из четырёх, `scope_key: string` (для `user_*` — `account_id`; для `global_*` — константа `all`), `day: Date`, `used: int`, `limit: int`. Тройка `(scope, scope_key, day)` уникальна; списание — ОДИН оператор |
...
| `job_attempt` | `video_id: UUID`, `stage: stt / select / render` — ЗАКРЫТЫЙ набор из трёх, `clip_id: UUID?` (у стадии `render` — клип, у остальных `NULL`), `attempt_no: int` (от 1, в пределах видео и стадии), `fence: int` (МОНОТОННЫЙ счётчик на видео, общий для трёх стадий: следующая попытка любой стадии получает номер больше любого выданного ранее), `status: running / succeeded / failed / deferred` — ЗАКРЫТЫЙ набор из четырёх (принято координатором, DEC-A-009), `unit: minutes / calls / none` (единица счёта расхода), `unit_count: int`, `provider: string?`, `model: string?`, `failure_reason: string?`, `started_at: Timestamp`, `finished_at: Timestamp?` |
...
INPUT: аккаунт, причина списания `reason` ∈ {`upload` — одна загрузка, `minutes` — минуты записи, `llm` — вызов модели выделения}, величина `n`, дата в `Europe/Moscow`.
OUTPUT: `granted`, либо `refused(scope)`, где `scope` — ОДИН из четырёх ключей канона, назвавший отказ. Вызов поставщика после `refused` не выполняется НИКОГДА.
STEPS:
1. На старте процесса: IF хотя бы один из ЧЕТЫРЁХ потолков не задан в окружении — 2 загрузки на аккаунт, 60 минут на аккаунт, 600 минут на всех, 20 вызовов на всех — THEN уронить запуск с сообщением, называющим НЕНАСТРОЕННЫЙ вызов. Пустая переменная не означает «ограничений нет»: она означает отказ (`model-cost-contract.md`, Правило №0).
2. Собрать ключи ПО ПРИЧИНЕ. `upload` → один: `(user_uploads, account_id, day)` с пределом 2. `minutes` → два: `(user_minutes, account_id, day)` с пределом 60 и `(global_minutes, 'all', day)` с пределом 600. `llm` → один: `(global_llm, 'all', day)` с пределом 20.
3. Все ключи одной причины списываются в ОДНОЙ транзакции, каждый — ОДНИМ оператором: `INSERT INTO quota_counter (scope, scope_key, day, used, limit) VALUES (…, :n, :limit) ON CONFLICT (scope, scope_key, day) DO UPDATE SET used = quota_counter.used + :n WHERE quota_counter.used + :n <= quota_counter.limit RETURNING used`. «Прочитать, потом записать» ЗАПРЕЩЕНО: две одновременные загрузки обе прочитают старое значение и обе пройдут. Последовательный тест зеленеет при обеих реализациях, различает их только конкурентный прогон.
4. IF ЛЮБОЙ оператор вернул пустой результат THEN откатить транзакцию ЦЕЛИКОМ — ни один счётчик этой причины не увеличен — и RETURN `refused` с названным `scope`, НЕ вызывая поставщика. Откат именно транзакцией, а не встречным уменьшением: параллельная попытка, попавшая между увеличением и уменьшением, получила бы отказ незаслуженно.
5. Отказ несёт `scope` и время возврата (ближайшая полночь `Europe/Moscow`). Четыре ключа дают ЧЕТЫРЕ разных текста: «загрузки на сегодня исчерпаны», «минуты на сегодня исчерпаны», «общий суточный объём исчерпан», «общий суточный лимит обработок исчерпан». Пользователь глобального потолка не виноват и должен это видеть.
...
COMPLEXITY: O(1) на ключ; не больше двух индексированных записей на причину.
...
4. `HEAD` объекта → `actual_bytes`. IF `actual_bytes > 2 000 000 000` THEN удалить объект, `failed(too_large)`, RETURN `invalid`. Это ВТОРАЯ проверка размера; первая была по заявленному значению, а заявленное клиентом — значение, которое не проверено.
5. Прочитать первые байты объекта диапазонным запросом и определить тип по МАГИЧЕСКИМ БАЙТАМ (MP4, MOV, WebM, M4A, MP3). Ни расширение, ни `Content-Type` не считаются: оба присылает клиент. IF тип не распознан THEN удалить объект, `failed(not_media)`, RETURN `invalid`.
6. `ffprobe` в ОТДЕЛЬНОМ процессе с таймаутом → длительность и наличие звуковой дорожки. IF дорожки нет THEN `failed(no_audio)`. IF длительность < 120 с THEN `failed(too_short)`. IF > 5400 с THEN `failed(too_long)`. Во всех трёх случаях минуты НЕ списываются (вызова поставщика не было), а списание `user_uploads` СОХРАНЯЕТСЯ: загрузка состоялась. Экран показывает «загрузить другой файл», а не «повторить»: тот же файл даст тот же отказ.
7. `minutes = ceil(duration_seconds / 60)`. Вызвать `CheckAndConsumeQuota(account, reason = minutes, n = minutes)` — ПОСЛЕ валидации и ДО постановки в очередь. IF `refused(scope)` THEN `failed(refused_user_minutes | refused_global_minutes)` по названному ключу, RETURN `refused(scope)`. Ключ `user_uploads` в этой транзакции НЕ трогается: он списан на шаге 4 `CreateVideo`, и второе списание было бы дефектом (контракт стоимости приведён к этому порядку, DEC-A-009).
8. В той же транзакции: `actual_bytes`, `duration_seconds`, `minutes_charged`, `status = queued`, `updated_at = now`.
9. ПОСЛЕ фиксации строки — поставить задание стадии `stt` (`LeaseAttempt`). Порядок обязателен: задание, поставленное раньше фиксации, может быть взято воркером, который записи ещё не увидит.
10. RETURN `202` с `video_id` и статусом.
COMPLEXITY: O(p) по числу частей плюс один `ffprobe`.
...
REALISES: —
INPUT: `video_id`, стадия ∈ {`stt`, `select`, `render`}, `clip_id?`, признак «попытка автоматическая либо по кнопке владельца».
OUTPUT: строка `job_attempt` с выданным `fence`, либо `exhausted`.
STEPS:
1. В ОДНОЙ транзакции прочитать `max(fence)` по всем попыткам ЭТОГО видео и `max(attempt_no)` по этой стадии (и клипу, если стадия `render`).
2. IF попытка АВТОМАТИЧЕСКАЯ AND `attempt_no` в ТЕКУЩЕЙ СЕРИИ уже равен 2 THEN RETURN `exhausted`: не больше двух автоматических попыток на стадию (ADR-001, п. 7). Попытка ПО КНОПКЕ владельца этим потолком не ограничена — её ограничивают квоты минут и вызовов, то есть деньги, а не счётчик: кнопка открывает НОВУЮ серию попыток (принято координатором, DEC-A-009).
3. `fence = max(fence) + 1` — счётчик МОНОТОННЫЙ на ВИДЕО, общий для трёх стадий: рендер клипа не должен получать номер, меньший чем у уже выданной попытки транскрипции того же видео, иначе «актуальность» перестаёт быть сравнимой.
4. Вставить `job_attempt` со `status = running`, `started_at = now`, `unit`/`unit_count` по стадии (`minutes` для `stt`, `calls` для `select`, `none` для `render`).
...
REALISES: —
INPUT: `object_key` оригинала, длительность записи.
OUTPUT: список частей `{путь, offset_seconds, duration_seconds}`, каждая ≤ 25 МБ.
STEPS:
1. Звук извлекается ПОТОКОВО: объект читается из хранилища в `ffmpeg -vn` и кодируется в сжатый формат. Полная копия двухгигабайтного оригинала на диск воркера транскрипции НЕ кладётся — на диске живут только части.
...
6. Каждая часть удаляется сразу после отправки поставщику; каталог очищается и после успеха, И после отказа.
...
STEPS:
1. Прочитать `video`; IF статус не `queued` и не `transcribing` THEN RETURN (задание устарело). Перевести `status = transcribing`, `updated_at = now`.
...
4. IF часть ответила таймаутом или ошибкой поставщика THEN повторить ЭТУ часть не более двух раз; КАЖДЫЙ повтор — новая строка расхода. Это и есть счёт по попыткам, и он объявлен в `model-cost-contract.md`.
...
4. Отказы по потолкам записываются ОТДЕЛЬНО, с названным `scope`: доля отказов по каждому из четырёх ключей — метрика недели, и общий счётчик «лимит исчерпан» её не даёт.
...
STEPS:
1. `CheckAndConsumeQuota(account, reason = llm, n = 1)` ДО вызова. IF `refused(global_llm)` THEN `failed(refused_global_llm)`, RETURN.
...
INPUT: ширина и высота кадра (1080×1920), домен продукта, `clip_link.code`.
OUTPUT: параметры фильтра надписи: текст, кегль, координаты, подложка.
STEPS:
1. Текст — «КлипМейкер · <домен>/c/<code>», где `<code>` — код короткой ссылки ИМЕННО ЭТОГО клипа. Метка без адреса — показ без измерения: метрика недели считается по переходам.
...
2. **Резерв диска ДО скачивания:** свободное место рабочего тома обязано быть ≥ 3 × `video.actual_bytes`. IF меньше THEN `job_attempt.status = deferred`, `clip.failure_reason = no_disk`, задание возвращается в очередь с задержкой, и объект НЕ скачивается — число обращений к хранилищу на этом пути равно нулю (ADR-006). Задание откладывается, а не падает на середине записи файла.
...
8. IF процесс завершился ошибкой THEN `failed(ffmpeg_failed)`; IF таймаутом THEN `failed(ffmpeg_timeout)`. Для автоматической попытки номер 2 задание возвращается в очередь; после второй — `clip.status = failed`.
9. Положить клип и превью в ПРИВАТНЫЙ бакет по серверным ключам. Публичной ссылки на объект не существует ни на одном шаге.
...
REALISES: SC-US-005-1, SC-US-007-1
...
4. IF `clip.status` не `done` OR `expires_at` в прошлом THEN `not_found`: срок хранения истёк, файла больше нет. Короткая ссылка `/c/{code}` при этом продолжает работать — она ведёт на лендинг, а не на файл.
...
5. При `отказе` вернуть причину СЛОВАМИ из закрытого списка и подходящее действие: «повторить» для причин, зависящих от прогона (`stt_failed`, `schema_violation`, `stalled`, `render_failed`, отказы по потолкам), «загрузить другой файл» для причин, зависящих от файла (`too_large`, `not_media`, `no_audio`, `too_short`, `too_long`) — тот же файл даст тот же отказ.
...
REALISES: SC-US-004-1, SC-US-012-2
INPUT: сессия, `video_id`.
...
5. ELSE `INSERT INTO growth_event (type, clip_link_id, ip_prefix, day, partner_code_id) VALUES ('link_view', …) ON CONFLICT (type, clip_link_id, ip_prefix, day) DO NOTHING RETURNING id`. Уникальность считает БАЗА: один префикс за сутки даёт единицу, сколько бы раз ни открыл. Счётчик `clip_link.unique_view_count` увеличивается ТОЛЬКО когда оператор вернул строку.
...
5. Вставить `guest_pack`: `code` ≥ 128 бит (адрес не перечисляется и не угадывается), `expires_at = now + 14 суток`, `host_partner_code_id` — личный код ведущего. Затем строки `guest_pack_clip` по отмеченным клипам.
6. Разметку спикеров делает ЧЕЛОВЕК: диаризации в неделе нет, галочки ставит ведущий.
7. `guest.send` — отдельное действие: помечает `sent_at` и пишет `growth_event(guest_sent)`. Отправка без нажатия не происходит.
...
STEPS:
1. Найти `guest_pack` по коду. IF не найден OR `revoked_at` заполнен OR `expires_at` в прошлом THEN `not_found` — ОДИН и тот же ответ на все три случая: различие сообщало бы, что страница когда-то была.
...
7. ELSE записать `growth_event(guest_opened)` один раз на пару `(guest_pack, ip_prefix)` в сутки; `first_opened_at`, если пусто.
...
3. Сторож: пакеты старше 14 суток с момента отправки закрываются тем же способом.
...
4. IF владелец кода — ЭТОТ аккаунт THEN записать `attribution` со `status = rejected`, `reject_reason = self_referral`, строку в аудит, и RETURN эту строку. Счётчики кабинета не растут. Самореферал — записанный исход, а не ошибка ввода.
5. Порядок силы источников ЗАКРЫТ: `explicit > guest_link > cookie`. Решение — одним оператором, а не чтением с последующей записью:
   ...
   - пустой результат — атрибуция есть: `UPDATE attribution SET partner_code_id = :new, source = :new_source, replaced_source = source, status = 'pending' WHERE account_id = :acc AND source IN (:строго более слабые)`. Одна затронутая строка — замена состоялась; ноль строк — RETURN `conflict`, потому что существующий источник равен новому или сильнее.
...
4. Короткая ссылка `/c/{code}` НЕ трогается и продолжает открывать лендинг: зритель чужой ленты не должен упираться в отказ из-за нашей уборки.
5. Экран клипов показывает оставшийся срок с первого дня (шаг 7 `ListClipsForScreen`), а не сообщает о нём после удаления.
6. Страховкой служит правило жизненного цикла бакета с БОЛЬШИМ сроком, чем прикладной: оно подчищает то, что приложение пропустило, и никогда не опережает его.
...
2. IF причина отказа из числа зависящих от ФАЙЛА (`too_large`, `not_media`, `no_audio`, `too_short`, `too_long`) THEN `conflict` с текстом «загрузите другой файл»: тот же файл даст тот же отказ, и повтор был бы оплаченной бессмыслицей.
3. Определить стадию продолжения по СОСТОЯНИЮ, а не по причине: нет `transcript` → `stt`; `transcript` есть, клипов нет → `select`; клипы есть → `render`, и только для тех, что не в `done`.
4. **Повтор ПРОДОЛЖАЕТ, а не начинает заново.** Готовый транскрипт не пересчитывается, и минуты за него второй раз не списываются. Продолжение со стадии `stt` — это НОВЫЙ вызов поставщика, и минуты списываются снова: счёт идёт по попыткам, и отменить это может только отсутствие вызова.
5. Продолжение со стадии `select` вызывает `CheckAndConsumeQuota(reason = llm)` заново: это новый оплаченный вызов.
6. `LeaseAttempt` для выбранной стадии с признаком «по кнопке владельца», новый фенс; предыдущие попытки этим самым становятся неактуальными, и их поздние результаты будут отброшены.
6а. **Нажатие открывает НОВУЮ СЕРИЮ попыток** (DEC-A-009): счётчик `attempt_no` автоматических повторов этой стадии обнуляется, и потолок «не больше двух» начинает действовать заново внутри новой серии. Ограничивает повтор не счётчик, а ДЕНЬГИ: каждая серия проходит через `CheckAndConsumeQuota`, и исчерпанные минуты либо вызовы останавливают её так же, как первую обработку. Счётчик, ограничивающий человека, который смотрит на свой отказ, наказывал бы за наш сбой; кончившаяся квота — честная граница, потому что за ней стоит счёт поставщика.
...
`429`, тело называет `scope` из четырёх ключей канона и время возврата), `conflict` (`409`),
...
**Публичные пути — ровно девять** (обработчики маршрутов, без tRPC; канон §5):
...
| 6 | `POST /api/upload/complete` | `Cookie: session`; `{ video_id, parts: [{ part_number, etag }] }` | `202` `{ video_id, status: 'queued' }` | `404` чужой · `409` статус не `uploading` · `invalid` слишком большой файл, не медиа, нет дорожки, вне 2–90 мин · `refused(user_minutes \| global_minutes)` |
| 7 | `POST /api/auth/login` | `{ email }` либо `{ token }` | `200` отправлено, либо `200` + cookie сессии | `invalid` непригодный адрес, неверный или истёкший токен · `429` частота |
| 8 | `POST /api/auth/logout` | `Cookie: session` | `200`, сессия погашена НА СЕРВЕРЕ | `401` сессии нет |
| 9 | `POST /api/auth/telegram` | сырой ID-токен | `200` + cookie сессии | `invalid` подпись не сошлась или срок истёк · `429` частота |

**Процедуры tRPC — ровно четырнадцать** (канон §5):
...
| 10 | `guest.send` | `{ guest_pack_id, channel: 'copy' \| 'telegram' }` | `{ sent_at, url }`, `growth_event(guest_sent)` | `404` · `409` пакет отозван либо истёк |
| 11 | `code.apply` | `{ code, source: 'explicit' \| 'guest_link' \| 'cookie' }` | `{ attribution_id, status: 'pending' \| 'rejected', source, replaced_source }` | `invalid` код не найден либо заблокирован — НИЧЕГО не изменено, отката к cookie нет · `conflict` существующий источник равен новому либо сильнее |
| 12 | `partner.dashboard` | `{ window: 'day' \| 'week' \| 'all' }` | `{ code, clicks, registrations, videos, clips_outside, guests, i, conv_pct, n_observations }` — пять счётчиков раздельно | `403` чужой код · `invalid` неизвестное окно |
| 13 | `interest.create` | `{ contact, source_screen: 'clip_card' \| 'guest_page' \| 'partner_dashboard' }` | `{ recorded: true }` | `invalid` пустой контакт либо источник вне набора |
| 14 | `account.delete` | `{ confirm: true }` | `{ accepted: true, erase_deadline }` | `invalid` без подтверждения · `conflict` удаление уже запущено |
...
stateDiagram-v2
    ...
    uploading --> queued : upload/complete — тип, длительность, минуты списаны
    uploading --> failed : too_large / not_media / no_audio / too_short / too_long / refused(*)
    queued --> transcribing : задание stt взято воркером
    ...
    selecting --> failed : no_fragments / schema_violation / refused(global_llm) / stalled
    rendering --> done : хотя бы один клип готов
    rendering --> failed : все клипы отказали / stalled
    failed --> queued : video.retry — продолжение со стадии падения
    failed --> transcribing : video.retry при готовом транскрипте
...
| Потолок | атомарный оператор вернул пустой результат | вся транзакция откатана, вызова поставщика НЕ было, `refused(scope)` | четыре разных текста по ключу плюс время возврата (полночь `Europe/Moscow`) |
...
| Нет места на диске | свободно меньше 3× размера оригинала | `deferred(no_disk)`, задание возвращается в очередь, объект НЕ скачивается | состояние остаётся `выполняется`: ожидание очереди — это работа, а не поломка |
...
Девять алгоритмов не заявляют ни одного `SC-US-nnn-k`, и это не пробел: `AuthRegisterAndLogin`,
`TelegramLogin`, `IngestFromUrl`, `LeaseAttempt`, `ExtractAndChunkAudio`, `Transcribe`,
`SendClipToTelegram`, `AntiFraudCodeBurst` и `RetryVideo` реализуют требования, у которых нет
пользовательского сценария приёмки — они проверяются Gherkin-сценариями раздела «Growth-требования»,
границами файла и сроками, а не историями. Обратная таблица пуста: ни один `REALISES` не указывает
на идентификатор, которого нет в спецификации.
