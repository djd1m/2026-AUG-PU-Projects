...
const input = (nodes: ReactElement<Props>[], type: string) => nodes.filter(n => n.props.type === type)[type === 'checkbox' ? 2 : 0]!;
...
it('default on; selected flag is sent and locked during upload and after failure', async () => {
  ...
  expect(vi.mocked(rpc).mock.calls[0]![1]).toMatchObject({ compact: true });
  expect(JSON.parse(storage.get('n5-upload-resume')!).compact).toBe(true);
  expect(input(render(), 'checkbox').props.disabled).toBe(true);
});
it.each([true, false, undefined])('resume restores locked compact %s for same file, another file unlocks', async value => {
  storage.set('n5-upload-resume', JSON.stringify({ key: 'key', fingerprint: 'podcast.mp3:24:123', compact: value }));
  ...
  await vi.waitFor(() => expect(vi.mocked(rpc)).toHaveBeenCalled());
  expect(vi.mocked(rpc).mock.calls[0]![1]).toMatchObject({ compact: value === true });
...
it.each([true, false])('file without Resume preserves current compact %s', value => {
