...
it('default false persists in SQL parameter, true passes; changed flag is 409 before S3', async () => {
  for (const teaser of [undefined, false, true]) {
    ...
    const call = service.create('account', randomUUID(), { filename: 'a.mp3', source: 'upload', declared_bytes: 24, teaser });
    if (teaser) { await expect(call).rejects.toMatchObject({ status: 409 }); expect(sign).not.toHaveBeenCalled(); }
