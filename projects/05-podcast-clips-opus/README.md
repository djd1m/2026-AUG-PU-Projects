...
docker compose --project-directory . --env-file .env --profile test run --rm --build test   # 857 тестов на реальных PostgreSQL, Redis, MinIO
...
| [`docs/features/README.md`](docs/features/README.md) | 26 фич и где лежит доказательство каждой |
...
| Phase 4 — Finalize | ✅ |
| Реализация | ✅ 12 фич MVP + 14 после живого прогона (кадр, субтитры, таймкоды, музыка, библиотека, пэк-шот, заголовок) + справедливость партнёрки + уплотнение пауз + смена музыки у клипа + прибор мобильной вёрстки + исправления мобильной вёрстки + тёмная тема + карточка клипа, все `done`; 857 тестов зелёные; прогон с нуля пройден (24.09.2026) |
