...
interface VideoRow {
  ...
  music: boolean; teaser: boolean; upload_part_size: number; upload_day: string; upload_enqueued_at: Date | null;
}
  ...
  async create(account: string, key: unknown, input: unknown): Promise<UploadData> {
    ...
    const claimed = await transaction(this.pool, async (tx) => {
      ...
      const claim = await tx.query<VideoRow>(`INSERT INTO video
        (id, account_id, idempotency_key, status, source, declared_bytes, upload_day, object_key, upload_part_size, music, teaser)
        VALUES ($1, $2, $3, 'uploading', 'upload', $4, $5, $6, $7, $8, $9)
        ON CONFLICT (account_id, idempotency_key) DO NOTHING RETURNING *, upload_day::text AS upload_day`,
      [id, account, key, BigInt(body.declared_bytes), moscowDay(now), objectKey, calculatePartSize(body.declared_bytes), body.music ?? false, body.teaser ?? false]);
      if (claim.rowCount) {
