...
interface Resume { key: string; fingerprint: string; music: boolean; teaser: boolean; videoId?: string; parts?: { part_number: number; etag: string }[] }
...
export function VideoUploader() {
  ...
  useEffect(() => { try { const value = sessionStorage.getItem(storageKey); if (value) { const saved = JSON.parse(value); resume.current = { ...saved, music: saved.music === true, teaser: saved.teaser === true }; } } catch { setMessage('Не удалось восстановить предыдущую загрузку. Выберите файл заново.'); }
  ...
  async function upload() {
    ...
    const current = resume.current?.fingerprint === fingerprint ? resume.current : { key: crypto.randomUUID(), fingerprint, music, teaser };
    save(current); const abort = new AbortController(); controller.current = abort; setBusy(true); setMessage('Подготавливаем загрузку…');
    try {
      const create = () => rpc<UploadData>('video.create', { filename: file.name, declared_bytes: file.size, source: 'upload', music: current.music, teaser: current.teaser }, true, current.key, abort.signal);
      if (!current.parts) {
  ...
  return <section className="upload-panel"><div><p className="eyebrow">НОВЫЙ ВЫПУСК</p><h2>Один разговор.<br />Несколько сильных моментов.</h2>
    ...
    <div className="upload-controls"><label htmlFor="source-file">Выберите видео или аудио</label>
      <input id="source-file" type="file" accept=".mp4,.mov,.webm,.m4a,.mp3" disabled={busy} onChange={e => { const selected = e.target.files?.[0] ?? null; setFile(selected);
        ...
        if (previous) { setMusic(previous.music); setTeaser(previous.teaser); } setMessage(''); }} />
