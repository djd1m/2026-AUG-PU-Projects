...
interface Resume { key: string; fingerprint: string; music: boolean; videoId?: string; parts?: { part_number: number; etag: string }[] }
...
export function VideoUploader() {
  ...
  useEffect(() => { try { const value = sessionStorage.getItem(storageKey); if (value) { const saved = JSON.parse(value); resume.current = { ...saved, music: saved.music === true }; } } catch { setMessage('Не удалось восстановить предыдущую загрузку. Выберите файл заново.'); }
  ...
  async function upload() {
    ...
    const current = resume.current?.fingerprint === fingerprint ? resume.current : { key: crypto.randomUUID(), fingerprint, music };
    save(current); const abort = new AbortController(); controller.current = abort; setBusy(true); setMessage('Подготавливаем загрузку…');
    try {
      const create = () => rpc<UploadData>('video.create', { filename: file.name, declared_bytes: file.size, source: 'upload', music: current.music }, true, current.key, abort.signal);
      if (!current.parts) {
  ...
  return <section className="upload-panel"><div><p className="eyebrow">НОВЫЙ ВЫПУСК</p><h2>Один разговор.<br />Несколько сильных моментов.</h2>
    ...
    <div className="upload-controls"><label htmlFor="source-file">Выберите видео или аудио</label>
      <input id="source-file" type="file" accept=".mp4,.mov,.webm,.m4a,.mp3" disabled={busy} onChange={e => { const selected = e.target.files?.[0] ?? null; setFile(selected);
        ...
        if (previous) setMusic(previous.music); setMessage(''); }} />
      ...
      <label><input type="checkbox" checked={music} disabled={busy || !!(file && resume.current?.fingerprint === `${file.name}:${file.size}:${file.lastModified}`)}
        onChange={e => setMusic(e.target.checked)} /> Добавить фоновую музыку</label>
      <small className="muted">Komiku — Everything is groovy, CC0</small>
