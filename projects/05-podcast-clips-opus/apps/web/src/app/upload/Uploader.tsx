...
interface Resume { key: string; fingerprint: string; videoId?: string; parts?: { part_number: number; etag: string }[] }
...
export function VideoUploader() {
  ...
  useEffect(() => { try { const value = sessionStorage.getItem(storageKey); if (value) resume.current = JSON.parse(value); } catch { setMessage('Не удалось восстановить предыдущую загрузку. Выберите файл заново.'); }
  ...
  async function upload() {
    ...
    const current = resume.current?.fingerprint === fingerprint ? resume.current : { key: crypto.randomUUID(), fingerprint };
    save(current); const abort = new AbortController(); controller.current = abort; setBusy(true); setMessage('Подготавливаем загрузку…');
    try {
      const create = () => rpc<UploadData>('video.create', { filename: file.name, declared_bytes: file.size, source: 'upload' }, true, current.key, abort.signal);
      if (!current.parts) {
  ...
  return <section className="upload-panel"><div><p className="eyebrow">НОВЫЙ ВЫПУСК</p><h2>Один разговор.<br />Несколько сильных моментов.</h2>
    ...
    <div className="upload-controls"><label htmlFor="source-file">Выберите видео или аудио</label>
      <input id="source-file" type="file" accept=".mp4,.mov,.webm,.m4a,.mp3" disabled={busy} onChange={e => { setFile(e.target.files?.[0] ?? null); setMessage(''); }} />
