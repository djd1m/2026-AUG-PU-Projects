...
export async function POST(request: Request) {
  ...
  try {
    ...
    return await fetchRequestHandler({ endpoint: '/api/trpc', req: bounded, router: appRouter, allowBatching: false,
      createContext: () => ({ account, idempotencyKey: request.headers.get('idempotency-key'), requestId, video: deps.video, retry: deps.retry, screen: deps.screen, links: deps.links }),
