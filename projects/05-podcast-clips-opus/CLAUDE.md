...
в начале клипа (ADR-013). Затем RT-002/RT-009 (ADR-014), уплотнение пауз (ADR-015), каталог музыки из 9 треков, прослушанных владельцем. Всего **21 фича**, тесты — **743 из 743** на настоящих PostgreSQL 16, Redis 7
...
3. **ADR** ([`docs/ADR.md`](docs/ADR.md)) — пятнадцать решений (ADR-001…015), у каждого Confirmation —
...
`music-library`, `teaser-headline`, `partner-fairness`, `pause-compaction`. Указатель с доказательствами — `docs/features/README.md`.
